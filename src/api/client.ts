...
export const BASE =
  ...
  /^https?:\/\/app\.deenlink\.org$/.test(window.location.origin)
...
function parseLooseJson<T>(text: string | null | undefined): T | null {
  ...
  if (idOnly && /success/i.test(text))
    return { status: "success", id: Number(idOnly[1]) } as unknown as T;
  return null;
}
...
async function request<T = Record<string, unknown>>(
  ...
  try {
    res = await fetch(`${BASE}${path}`, {
      method:
        opts.method ?? (opts.body !== undefined || opts.form ? "POST" : "GET"),
      headers,
  ...
  const _isWrite =
    opts.method === "POST" || opts.body !== undefined || !!opts.form;
...
export async function fetchCsrf(): Promise<string | null> {
  const r = await request<{ status: string; csrf_token?: string }>(
    "/api/auth/csrf.php",
  );
...
export async function login(
  ...
  if (r.networkError && FORCE_DEMO)
    await storage.setItem("dl.demoSession", "1");
...
export async function feed(
  ...
  const r = await request<FeedResponse>(
    `/api/feed/get_posts.php?tab=${tab}&limit=20&cursor=${cursor}`,
  );
...
export async function getComments(
  ...
  const r = await request<{ status?: string; comments?: ServerComment[] }>(
    `/api/feed/get_comments.php?post_id=${postId}`,
    { auth: true },
  );
...
export async function addComment(
  ...
  }>("/api/feed/add_comment.php", {
    method: "POST",
    body: { post_id: postId, text },
    auth: true,
  });
  return r.ok && r.data.comment_id
    ? { id: r.data.comment_id as number, count: r.data.comment_count ?? 0 }
    : null;
}
...
export async function toggleCommentLike(
  ...
  const r = await request<{
    ...
    liked?: boolean;
    like_count?: number;
  }>("/api/feed/toggle_comment_like.php", {
    method: "POST",
    body: { comment_id: commentId, desired_liked: desired },
    auth: true,
  });
  return r.ok
    ? { liked: !!r.data.liked, like_count: r.data.like_count ?? 0 }
    : null;
}
export async function toggleReplyLike(
  ...
  const r = await request<{
    ...
    liked?: boolean;
    like_count?: number;
  }>("/api/feed/toggle_reply_like.php", {
    method: "POST",
    body: { reply_id: replyId, desired_liked: desired },
    auth: true,
  });
  return r.ok
    ? { liked: !!r.data.liked, like_count: r.data.like_count ?? 0 }
...
export async function deletePost(postId: number): Promise<boolean> {
  const r = await request<{ status?: string }>("/api/feed/delete_post.php", {
    method: "POST",
    body: { post_id: postId },
    auth: true,
  });
...
export async function groupDeletePost(postId: number): Promise<boolean> {
  const r = await request<{ status?: string }>("/api/groups/delete_post.php", {
    method: "POST",
    body: { post_id: postId },
    auth: true,
  });
...
export async function publicGet<T = Record<string, unknown>>(
  path: string,
): Promise<ApiResult<T>> {
...
export async function fetchArticles(): Promise<ServerArticle[] | null> {
  try {
    const r = await request<{ status?: string; items?: ServerArticle[] }>(
      "/api/content/articles.php",
      { auth: false },
    );
    if (r.ok && Array.isArray(r.data.items) && r.data.items.length)
      return r.data.items.filter((a) => a.is_active !== 0);
  } catch {}
  return null;
}
...
export async function fetchJokes(): Promise<ServerJoke[] | null> {
  try {
    const r = await request<{ status?: string; items?: ServerJoke[] }>(
      "/api/content/jokes.php",
      { auth: false },
    );
    if (r.ok && Array.isArray(r.data.items) && r.data.items.length)
      return r.data.items.filter((j) => j.is_active !== 0);
  } catch {}
  return null;
}
...
export async function reportPost(
  ...
  const r = await request<{ status?: string }>("/api/feed/report_post.php", {
    method: "POST",
...
export async function groupsList(q?: string): Promise<GroupRow[] | null> {
  ...
  const r = await request<{ status?: string; groups?: GroupRow[] }>(
    `/api/groups/list.php${q && q.trim() ? `?q=${encodeURIComponent(q.trim().slice(0, 60))}` : ""}`,
    { auth: true },
  );
  return r.ok && Array.isArray(r.data.groups) ? r.data.groups : null;
}
export async function groupGet(id: number): Promise<GroupRow | null> {
  const r = await request<{ status?: string; group?: GroupRow }>(
    `/api/groups/get.php?id=${id}`,
    { auth: true },
  );
...
> {
  ...
  }>("/api/settings/public.php", { auth: true });
...
export async function groupJoinRequests(
  ...
  const r = await request<{ status?: string; requests?: GroupJoinRequest[] }>(
    "/api/groups/members.php",
    {
      method: "POST",
      body: { group_id: groupId, action: "requests" },
      auth: true,
    },
  );
  return r.ok && Array.isArray(r.data.requests) ? r.data.requests : [];
}
export async function groupJoinDecide(
  groupId: number,
  userId: number,
  approve: boolean,
  ...
  return r.ok
    ? { ok: true }
    : { ok: false, message: r.data?.message || "Please try again." };
}
/* pass 83-28 — join a closed group → the server answers status:'requested' */
export async function groupJoinRich(
  groupId: number,
  join: boolean,
): Promise<"joined" | "requested" | "left" | "failed"> {
  const r = await request<{ status?: string }>("/api/groups/join.php", {
    method: "POST",
    body: { group_id: groupId, join },
    auth: true,
  });
...
export async function activeAnnouncement(): Promise<{
  ...
  const r = await request<{
    ...
    dismiss_key?: string;
  }>("/api/announcements/active.php", { auth: true });
  if (!r.ok || !r.data) return null;
  return {
    item: r.data.announcement ?? null,
    dismissKey: r.data.dismiss_key ?? "",
  };
}

export async function groupPrivacy(): Promise<boolean | null> {
  const r = await request<{ status?: string; allow_group_add?: number }>(
    "/api/users/group_privacy.php",
    { auth: true },
  );
  if (r.ok && typeof r.data.allow_group_add !== "undefined")
    return Number(r.data.allow_group_add) !== 0;
  return null;
}
...
export async function setGroupPrivacy(allow: boolean): Promise<boolean | null> {
  const r = await request<{ status?: string; allow_group_add?: number }>(
    "/api/users/group_privacy.php",
    { method: "POST", body: { allow: allow ? 1 : 0 }, auth: true },
  );
  if (r.ok && typeof r.data.allow_group_add !== "undefined")
    return Number(r.data.allow_group_add) !== 0;
  return null;
...
export async function groupMembers(
  ...
  ok: boolean;
  message?: string;
  denial?: { username: string; full_name: string } | null;
}> {
  const r = await request<{
    status?: string;
    message?: string;
...
export async function groupCreate(data: {
  ...
  const r = await request<{ status?: string; id?: number }>(
  ...
  for (const p of posts) {
    ...
    if (Array.isArray(p.media)) {
      p.media = p.media.map((m) => {
        const mm = m as {
          url?: unknown;
          image_url_1080?: unknown;
          image_url_360?: unknown;
        };
        if (mm.url == null && mm.image_url_1080 != null) {
          return {
            type: "image",
            url: absMedia(String(mm.image_url_1080)),
            thumb_url: absMedia(String(mm.image_url_360 ?? mm.image_url_1080)),
          };
        }
        return m;
      });
    ...
    if (typeof au === "string" && au)
      (p as { audio_url?: string }).audio_url = absMedia(au);
    ...
    if (!p.video_url && Array.isArray(p.media)) {
      const vm = p.media.find(
        (m) => (m as { video_url?: unknown }).video_url != null,
      ) as { video_url?: unknown } | undefined;
      if (vm && typeof vm.video_url === "string" && vm.video_url)
        p.video_url = absMedia(vm.video_url);
    }
    if (Array.isArray(p.media)) {
      p.media = p.media.map((m) => {
...
export async function groupPosts(
  id: number,
): Promise<import("@/api/types").Post[] | null> {
  const r = await request<{
    status?: string;
    posts?: import("@/api/types").Post[];
  }>(`/api/groups/posts.php?id=${id}`, { auth: true });
  ...
  if (hasMedia || yt) {
    ...
    for (const img of (images ?? []).slice(0, 5)) {
      ...
      if (typeof window !== "undefined" && small.startsWith("blob:")) {
        ...
        form.append(
          "images[]",
          new File([blob], img.name ?? "photo.jpg", {
    ...
    if (audio) {
      if (typeof window !== "undefined" && audio.uri.startsWith("blob:")) {
        ...
        form.append(
          "audio",
          new File([blob], audio.name ?? "audio.mp3", {
    ...
    if (video) {
      if (typeof window !== "undefined" && video.uri.startsWith("blob:")) {
        ...
        form.append(
          "video",
          new File([blob], video.name ?? "video.mp4", {
    ...
    const r = hasMedia
      ? await uploadForm<{ status?: string; id?: number; message?: string }>(
          "/api/groups/create_post.php",
          form,
          onProgress,
        )
      : await request<{ status?: string; id?: number; message?: string }>(
          "/api/groups/create_post.php",
    ...
    return r.ok && r.data && r.data.id
      ? { id: r.data.id as number, message: undefined }
      : { id: null, message: r.data?.message };
  }
  const r = await request<{ status?: string; id?: number; message?: string }>(
    "/api/groups/create_post.php",
    {
      method: "POST",
      body: {
        ...
        ...(opts.length >= 2 ? { poll_options: opts } : {}),
      },
      auth: true,
    },
  );
...
export async function videosUploadReel(
  ...
  form.append(
    "description",
    caption.trim().slice(0, 700) || "New video on DeenLink",
  );
  if (typeof window !== "undefined" && file.uri.startsWith("blob:")) {
    const blob = await fetch(file.uri).then((r) => r.blob());
    form.append(
      "video_file",
      new File([blob], file.name ?? "video.mp4", {
        type: blob.type || "video/mp4",
      }),
    );
  } else {
...
export async function votePoll(
  ...
  const r = await request<PollRow & { status?: string }>(
    "/api/feed/poll_vote.php",
    {
      method: "POST",
      body: { post_id: postId, option_id: optionId },
      auth: true,
    },
  );
  return r.ok && Array.isArray((r.data as PollRow).options)
    ? (r.data as PollRow)
    : null;
}
...
export async function linkPreview(url: string): Promise<LinkPreview | null> {
  const r = await request<LinkPreview & { status?: string }>(
    `/api/feed/link_preview.php?url=${encodeURIComponent(url)}`,
  );
...
export async function getConnections(
  tab: "following" | "followers" | "suggestions",
  q = "",
): Promise<{
  items: ConnectionRow[];
  ...
  const r = await request<{
    ...
    `/api/users/get_connections.php?tab=${tab}${q ? `&q=${encodeURIComponent(q)}` : ""}`,
    { auth: true },
  );
  return r.ok && Array.isArray(r.data.items)
    ? {
        items: r.data.items,
        counts: r.data.counts ?? { followers: 0, following: 0 },
      }
    : null;
}
export async function toggleFollow(
  ...
  const r = await request<{ status?: string }>("/api/users/toggle_follow.php", {
    method: "POST",
    body: { user_id: userId, desired_following: desired },
    auth: true,
  });
  return r.ok;
}
...
export async function searchAccounts(
  ...
  const r = await request<{ status?: string; results?: AccountResult[] }>(
    `/api/users/search_accounts.php?q=${encodeURIComponent(q)}&limit=${limit}`,
    { auth: true },
  );
...
export async function searchPosts(
  ...
  const r = await request<{ status?: string; posts?: Post[] }>(
    `/api/feed/search_posts.php?q=${encodeURIComponent(q)}&limit=${limit}&cursor=${cursor}`,
  );
  if (r.ok && Array.isArray(r.data.posts)) {
    return r.data.posts;
  }
...
export async function notificationsList(
  ...
  const r = await request<{ status?: string; notifications?: NotifRow[] }>(
    `/api/notifications/list.php?limit=${limit}`,
    { auth: true },
  );
...
export async function notificationsMarkAllRead(): Promise<void> {
  await request("/api/notifications/mark_read.php", {
    method: "POST",
    body: { all: true },
    auth: true,
  });
...
export async function bookmarksList(
  ...
  const r = await request<{ status?: string; items?: BookmarkItem[] }>(
    `/api/bookmarks/list.php${kind ? `?kind=${encodeURIComponent(kind)}` : ""}`,
    { auth: true },
  );
...
export async function bookmarkToggle(
  ...
  const r = await request<{ status?: string; bookmarked?: boolean }>(
    "/api/bookmarks/toggle.php",
    { method: "POST", body: { kind, item_id: itemId, payload }, auth: true },
  );
  if (r.ok && typeof r.data.bookmarked === "boolean") {
    return { bookmarked: r.data.bookmarked };
  }
  return null;
}
...
export async function deenpointsHistory(
  limit = 40,
): Promise<{ balance: number; events: PointsEvent[] } | null> {
  const r = await request<{
    status?: string;
    balance?: number;
    events?: PointsEvent[];
  }>(`/api/deenpoints/history.php?limit=${limit}`, { auth: true });
...
export type PointsPricing = {
  price_per_point_ngn: number;
  currency: string;
  rate_ngn_to_currency: number;
  supported_currencies?: string[];
  country?: string;
};

export async function pointsQuote(): Promise<PointsPricing | null> {
  const r = await request<{ status?: string; pricing?: PointsPricing }>(
    "/api/payments/flutterwave/quote_deenpoints.php",
  );
  if (r.ok && r.data.pricing) {
    return r.data.pricing;
  }
  return null;
}

/* pass 80 — display currency by account country (see lib/currency.ts). */
export async function fxQuote(): Promise<{
  currency: string;
  rate: number;
} | null> {
  const r = await request<{
    status?: string;
    currency?: string;
    rate_usd_to_currency?: number;
  }>("/api/payments/fx_quote.php");
  if (r.ok && r.data.currency)
    return {
      currency: r.data.currency,
      rate: Number(r.data.rate_usd_to_currency) || 1,
    };
  return null;
}
...
export async function pointsInit(
  points: number,
  opts?: { redirect?: boolean; redirectUrl?: string },
): Promise<{
  checkout?: FlwCheckout;
  mode?: string;
  tx_ref?: string;
  redirect_url?: string;
  message?: string;
  ...
  const r = await request<{
    ...
    redirect_url?: string;
    message?: string;
  }>("/api/payments/flutterwave/init_deenpoints.php", {
    method: "POST",
    body,
    auth: true,
  });
...
export async function pointsVerify(
  txRef: string,
): Promise<{ ok: boolean; balance?: number; message?: string } | null> {
  const r = await request<{
    status?: string;
    message?: string;
    deenpoints_balance?: number;
  }>("/api/payments/flutterwave/verify.php", {
    method: "POST",
    body: { tx_ref: txRef },
    auth: true,
  });
  if (r.ok && r.data.status === "success") {
    return {
      ok: true,
      balance: r.data.deenpoints_balance,
      message: r.data.message,
    };
  }
...
export async function donationInit(
  donationType: string,
  amount: number,
  opts?: {
    redirect?: boolean;
  ...
  const r = await request<{ status?: string } & PayInit>(
    "/api/payments/flutterwave/init_donation.php",
...
export async function quotePremium(): Promise<PremiumQuote | null> {
  const r = await request<{ status?: string } & PremiumQuote>(
    "/api/payments/flutterwave/quote_premium.php",
    { auth: true },
  );
  if (r.ok && r.data.status === "success") {
    return r.data;
...
export async function premiumInit(
  plan: "monthly" | "annual",
  opts?: { redirect?: boolean; redirectUrl?: string },
): Promise<PayInit | null> {
  ...
  const r = await request<{ status?: string } & PayInit>(
    "/api/payments/flutterwave/init_premium.php",
    { method: "POST", body, auth: true },
  );
  if (r.ok && r.data.status === "success") {
    return r.data;
...
export async function myQuestions(): Promise<{
  ...
  const r = await request<{
    ...
    questions?: MyQuestion[];
    counts?: Record<string, number>;
  }>("/api/questions/my_list.php", { auth: true });
...
export async function askUnreadCount(): Promise<number> {
  const r = await request<{ status?: string; unread_answered_count?: number }>(
    "/api/questions/unread_answered_count.php",
    { auth: true },
  );
...
export async function scholarRespond(
  ...
  const r = await request<{ status?: string }>("/api/questions/respond.php", {
    method: "POST",
    body: { question_id: questionId, action, [key]: text },
    auth: true,
  });
  return r.ok && r.data.status === "success";
}
...
export async function shopProducts(
  ...
  const r = await request<{ status?: string; products?: ShopProduct[] }>(
    `/api/shop/products.php${category && category !== "all" ? `?category=${encodeURIComponent(category)}` : ""}`,
  );
  return r.ok && Array.isArray(r.data.products)
    ? r.data.products.map(normalizeShopProduct)
    : null;
}
export async function shopProduct(id: number): Promise<ShopProduct | null> {
  const r = await request<{ status?: string; product?: ShopProduct }>(
    `/api/shop/product.php?id=${id}`,
  );
...
export async function shopSearch(q: string): Promise<ShopProduct[] | null> {
  const r = await request<{ status?: string; products?: ShopProduct[] }>(
    `/api/shop/search.php?q=${encodeURIComponent(q)}`,
  );
...
export async function shopCartAction(
  action: "add" | "remove" | "qty",
  productId: number,
  qty = 1,
  variantId?: number | null,
): Promise<boolean> {
  const r = await request<{ status?: string }>("/api/shop/cart.php", {
    method: "POST",
    body: {
      action,
...
export async function shopCheckout(info: {
  name: string;
  email: string;
  phone: string;
  country: string;
  ...
  address: string;
  note: string;
}): Promise<{ order_id: number; total: number } | null> {
  const r = await request<{
    status?: string;
    order_id?: number;
    total?: number;
  }>("/api/shop/checkout.php", { method: "POST", body: info, auth: true });
  return r.ok && r.data.order_id
    ? { order_id: Number(r.data.order_id), total: Number(r.data.total ?? 0) }
    : null;
}
/** pass 79 — start the Flutterwave payment for a shop order (web inline / native hosted). */
export async function shopPayInit(
  orderId: number,
  opts?: { redirect?: boolean; redirectUrl?: string },
): Promise<PayInit | null> {
  ...
  const r = await request<{ status?: string } & PayInit>(
    "/api/payments/flutterwave/init_shop.php",
    { method: "POST", body, auth: true },
  );
  if (r.ok && r.data.status === "success") {
    return r.data;
...
export async function shopOrders(): Promise<ShopOrder[] | null> {
  const r = await request<{ status?: string; orders?: ShopOrder[] }>(
    "/api/shop/orders.php",
    { auth: true },
  );
...
export async function blockUser(
  ...
  const r = await request<{ status?: string; blocked?: boolean }>(
    "/api/users/block_action.php",
    {
      method: "POST",
      body: { username, action: block ? "block" : "unblock" },
      auth: true,
    },
  );
  return r.ok && r.data.status === "success";
...
export async function myBlocks(): Promise<BlockedAccount[] | null> {
  const r = await request<{ status?: string; blocks?: BlockedAccount[] }>(
    "/api/users/blocks_list.php",
    { auth: true },
  );
  if (r.ok && Array.isArray(r.data.blocks)) {
...
export async function profileAvatars(
  gender?: string,
): Promise<ProfileAvatar[] | null> {
  const qs = gender ? `?gender=${encodeURIComponent(gender)}` : "";
  const r = await request<{ status?: string; avatars?: ProfileAvatar[] }>(
...
export async function wallpaperStore(): Promise<StoreWallpaper[] | null> {
  const r = await request<{ status?: string; wallpapers?: StoreWallpaper[] }>(
    "/api/wallpapers/list.php",
    { auth: true },
  );
  if (r.ok && Array.isArray(r.data.wallpapers)) {
    return r.data.wallpapers;
  }
  return null;
}
...
export async function wallpaperUnlock(wallpaperId: number): Promise<{
  ...
  const r = await request<{
    ...
    new_balance?: number;
    message?: string;
  }>("/api/wallpapers/unlock.php", {
    method: "POST",
    body: { wallpaper_id: wallpaperId },
    auth: true,
  });
  if (r.ok && r.data.status === "success") {
    return { ok: true, spent: r.data.spent, new_balance: r.data.new_balance };
  }
...
export async function getUserProfile(
  ...
  const r = await request<{ status?: string; user?: PublicProfile }>(
...
export async function toggleLike(
  ...
  const r = await request<{
    ...
    method: "POST",
    body: { post_id: postId, desired_liked: desired },
  });
  if (r.ok)
    return {
      like_count: r.data.like_count ?? 0,
      liked_by_me: r.data.liked_by_me ?? desired,
    };
...
function uploadForm<T>(
  ...
  onProgress?: (frac: number) => void,
  retried?: boolean,
): Promise<{ ok: boolean; data: T | null; networkError?: boolean }> {
  return new Promise((resolve) => {
    try {
      ...
      if (session && typeof window === "undefined")
        xhr.setRequestHeader("Cookie", `deenlink_session=${session}`);
      xhr.upload.onprogress = (e) => {
        if (e.lengthComputable && onProgress) {
          onProgress(Math.max(0, Math.min(1, e.loaded / Math.max(1, e.total))));
        }
      };
      xhr.onload = () => {
        let data: T | null = null;
        data = parseLooseJson<T>(
          xhr.responseText,
        ); /* pass 83-35 — warning-tolerant */
        ...
        if (
          ...
          void fetchCsrf().then(() =>
            resolve(uploadForm<T>(url, form, onProgress, true)),
          );
          return;
        }
...
export async function createPost(
  ...
  if (pollOptions && pollOptions.length >= 2)
    form.append("poll_options", JSON.stringify(pollOptions.slice(0, 6)));
  ...
  const r = hasMedia
    ? await uploadForm<{
        ...
        post_id?: number;
        id?: number;
      }>("/api/feed/create_post.php", form, onProgress)
    : await request<{
        status?: string;
        post?: Post;
        post_id?: number;
        id?: number;
      }>("/api/feed/create_post.php", { method: "POST", form });
  if (r.ok && r.data)
    return {
      ok: true,
      post: r.data.post,
      id: r.data.post_id ?? r.data.id ?? null,
    };
  return { ok: false };
}
...
export async function videos(
  type: "daily" | "reel" | "all" = "daily",
): Promise<Video[]> {
  const r = await request<{
    status?: string;
    videos?: Video[];
    items?: Video[];
  }>(`/api/videos/list.php?type=${type}&limit=20&source=homepage`);
...
/* ---- videos engagement ---- */
export async function videosLike(
  videoId: number,
  desired?: boolean,
): Promise<{ liked: boolean; like_count: number } | null> {
  ...
  const r = await request<{
    ...
    liked?: boolean;
    like_count?: number;
  }>("/api/videos/toggle_like.php", { method: "POST", body, auth: true });
  if (r.ok && r.data.status === "success" && typeof r.data.liked === "boolean")
    return { liked: r.data.liked, like_count: Number(r.data.like_count ?? 0) };
  return null;
}
...
export async function videosComments(
  videoId: number,
): Promise<VideoComment[] | null> {
  const r = await request<{ status?: string; comments?: VideoComment[] }>(
    `/api/videos/list_comments.php?video_id=${videoId}`,
    { auth: true },
  );
...
export async function videosCommentAdd(
  videoId: number,
  text: string,
  parentId?: number,
): Promise<{ comment_id: number; comment_count: number } | null> {
  ...
  const r = await request<{
    ...
    comment_id?: number;
    comment_count?: number;
  }>("/api/videos/add_comment.php", { method: "POST", body, auth: true });
  if (r.ok && r.data.status === "success" && r.data.comment_id)
    return {
      comment_id: Number(r.data.comment_id),
      comment_count: Number(r.data.comment_count ?? 0),
    };
  return null;
}

export async function videosCommentDelete(
  videoId: number,
  commentId: number,
): Promise<number | null> {
  const r = await request<{ status?: string; comment_count?: number }>(
    "/api/videos/delete_comment.php",
    {
      method: "POST",
      body: { video_id: videoId, comment_id: commentId },
      auth: true,
    },
  );
  if (r.ok && r.data.status === "success")
    return Number(r.data.comment_count ?? 0);
  return null;
}

export async function videosCommentLike(
  commentId: number,
  desired?: boolean,
): Promise<{ liked: boolean; like_count: number } | null> {
  ...
  const r = await request<{
    ...
    liked?: boolean;
    like_count?: number;
  }>("/api/videos/toggle_comment_like.php", {
    method: "POST",
    body,
    auth: true,
  });
...
export async function videosSave(
  videoId: number,
  desired?: boolean,
): Promise<boolean | null> {
  const body: Record<string, unknown> = { video_id: videoId };
  if (desired !== undefined) body.desired_saved = desired;
  const r = await request<{ status?: string; saved?: boolean }>(
    "/api/videos/bookmark.php",
    { method: "POST", body, auth: true },
  );
  if (r.ok && r.data.status === "success" && typeof r.data.saved === "boolean")
    return r.data.saved;
  return null;
}

export async function videosView(videoId: number): Promise<void> {
  await request("/api/videos/add_view.php", {
    method: "POST",
...
export async function videosReport(
  videoId: number,
  reason: string,
  hideAccount = false,
): Promise<boolean> {
  const r = await request<{ status?: string }>("/api/videos/report.php", {
    method: "POST",
    body: { video_id: videoId, reason, hide_account: hideAccount },
    auth: true,
  });
  return r.ok && r.data.status === "success";
}

export async function videosNotInterested(
  videoId: number,
  reason = "",
): Promise<boolean> {
  const r = await request<{ status?: string }>(
    "/api/videos/not_interested.php",
    { method: "POST", body: { video_id: videoId, reason }, auth: true },
  );
  return r.ok && r.data.status === "success";
}
...
export async function videosSearch(
  ...
  const r = await request<{ status?: string; videos?: Video[] }>(
    `/api/videos/search.php?q=${encodeURIComponent(q)}&limit=${limit}`,
    { auth: true },
  );
...
export async function courseGet(
  courseId?: number,
  slug?: string,
): Promise<ServerCourse | null> {
  const qs = courseId
    ? `course_id=${courseId}`
    : `slug=${encodeURIComponent(slug ?? "")}`;
  const r = await request<{ status?: string; course?: ServerCourse }>(
    `/api/courses/get.php?${qs}`,
    { auth: true },
  );
...
export async function courseEnroll(
  ...
  const r = await request<{ status?: string; course?: ServerCourse }>(
    ...
    { method: "POST", body: { course_id: courseId }, auth: true },
  );
  if (r.ok && r.data.status === "success") return r.data.course ?? null;
  return null;
}

export async function courseUnlockPoints(
  courseId: number,
  ...
  if (r.ok && r.data.status === "success") {
    const bal = Number(
      r.data.deenpoints_balance ?? r.data.new_balance ?? r.data.balance ?? NaN,
    );
    return {
      ok: true,
  ...
  return {
    ok: false,
    message: r.data.message ?? "Could not unlock this course",
  };
}

export async function courseQuizComplete(
  courseId: number,
  score: number,
  total: number,
): Promise<{
  ...
  const r = await request<{
    ...
    certificate?: Record<string, unknown> | null;
    message?: string;
  }>("/api/courses/quiz_complete.php", {
    method: "POST",
    body: { course_id: courseId, score, total },
    auth: true,
  });
  if (r.ok && r.data.status === "success")
    return {
      ...
      percent: Number(r.data.percent ?? 0),
      certificate: r.data.certificate ?? null,
    };
  return { ok: false, message: r.data.message ?? "Could not save quiz result" };
}

export async function courseCompleteLesson(
  courseId: number,
  lessonId: number,
): Promise<{
  ok: boolean;
  ...
  const r = await request<{
    ...
    message?: string;
    certificate?: Record<string, unknown> | null;
  }>("/api/courses/complete_lesson.php", {
    method: "POST",
...
export async function courseCertificate(
  ...
  }>(`/api/courses/certificate.php?course_id=${courseId}`, { auth: true });
...
export async function donationHistory(
  page = 1,
  perPage = 20,
): Promise<{ items: DonationRow[]; total: number } | null> {
  const r = await request<{
    status?: string;
    items?: DonationRow[];
    total?: number;
  }>(`/api/donations/my_history.php?page=${page}&per_page=${perPage}`, {
    auth: true,
  });
  if (r.ok && Array.isArray(r.data.items))
    return {
      items: r.data.items,
      total: Number(r.data.total ?? r.data.items.length),
    };
  return null;
}

export async function donationSummary(): Promise<{
  total: number;
  count: number;
  currency: string;
} | null> {
  const r = await request<{
    ...
    count?: number;
    currency?: string;
  }>("/api/donations/my_summary.php", { auth: true });
  if (r.ok && r.data.status === "success")
    return {
      total: Number(r.data.total ?? 0),
      count: Number(r.data.count ?? 0),
      currency: String(r.data.currency ?? "NGN"),
    };
  return null;
}
...
export async function quranReciters(): Promise<{
  reciters: ServerReciter[];
  balance: number;
} | null> {
  const r = await request<{
    status?: string;
    reciters?: Array<ServerReciter & { key?: string }>;
    user?: { deenpoints_balance?: number };
    balance?: number;
  }>("/api/quran/reciters.php", { auth: true });
  if (r.ok && Array.isArray(r.data.reciters)) {
    /* the endpoint emits `key`; normalize to reciter_key for the client */
    const reciters = r.data.reciters.map((x) => ({
      ...x,
      reciter_key: String(x.reciter_key ?? x.key ?? ""),
    }));
    return {
      reciters,
      balance: Number(r.data.user?.deenpoints_balance ?? r.data.balance ?? 0),
    };
  }
  return null;
}
...
export async function quranUnlockReciter(
  ...
  const r = await request<{
    ...
    message?: string;
    new_balance?: number;
  }>("/api/quran/unlock_reciter.php", {
    method: "POST",
    body: { reciter_key: reciterKey },
    auth: true,
  });
  if (r.ok && r.data.status === "success") {
    const bal = Number(r.data.new_balance ?? NaN);
  ...
  return {
    ok: false,
    message: r.data.message ?? "Could not unlock this reciter",
  };
}

export async function quranStreak(): Promise<{
  current: number;
  best: number;
} | null> {
  const r = await request<{
  ...
  }>("/api/quran/streak.php", { auth: true });
  if (r.ok && r.data.streak)
    return {
      current: Number(r.data.streak.current ?? 0),
      best: Number(r.data.streak.best ?? 0),
    };
  return null;
}

export async function quranStreakLog(): Promise<void> {
  await request("/api/quran/streak.php", {
    method: "POST",
    body: {},
    auth: true,
  }).catch(() => {});
...
export async function videosRepost(
  videoId: number,
  action: "repost" | "undo" | "toggle" = "toggle",
): Promise<{ reposted: boolean; repost_count: number } | null> {
  const r = await request<{
    status?: string;
    reposted?: boolean;
    repost_count?: number;
  }>("/api/videos/repost.php", {
    method: "POST",
    body: { video_id: videoId, action },
    auth: true,
  });
  if (
    r.ok &&
    r.data.status === "success" &&
    typeof r.data.reposted === "boolean"
  ) {
    return {
      reposted: r.data.reposted,
      repost_count: Number(r.data.repost_count ?? 0),
    };
  }
  return null;
}
...
export async function userPosts(userId?: number): Promise<Post[]> {
  ...
  if (userId != null) {
    const r = await request<{ status?: string; posts?: Post[]; data?: Post[] }>(
      `/api/feed/get_user_posts.php?user_id=${userId}&limit=50`,
    );
    if (r.ok) {
...
export async function publicDonationSummary(
  userId?: number,
  username?: string,
  ...
  const r = await request<{
    ...
    count?: number;
    currency?: string;
  }>(`/api/donations/user_summary.php?${q}`, { auth: true });
  if (r.ok && r.data.status === "success") {
    return {
      total: Number(r.data.total ?? 0),
      count: Number(r.data.count ?? 0),
...
export async function updateProfile(payload: {
  ...
  if (FORCE_DEMO) {
    return {
      ok: true,
      user: { ...MOCK_USER, ...payload } as User,
      message: "Saved (demo mode)",
    };
  }
  const r = await request<{ status?: string; message?: string; user?: User }>(
...
export async function uploadProfileImage(
  ...
  if (FORCE_DEMO)
    return { ok: false, message: "Photo upload needs the live API" };
  ...
  const r = await request<{
    ...
    message?: string;
  }>("/api/users/upload_profile_image.php", {
    method: "POST",
    form,
  });
...
export async function getSecurityQuestion(identifier: string): Promise<{
  ...
  if (FORCE_DEMO)
    return { ok: false, message: "Account recovery needs the live API" };
  const r = await request<{
    status?: string;
    found?: boolean;
    security_question?: string;
    message?: string;
  }>("/api/auth/get_security_question.php", {
    method: "POST",
    body: { identifier },
  });
  if (r.ok)
    return {
      ok: true,
      found: r.data.found,
      question: r.data.security_question,
      message: r.data.message,
    };
  return { ok: false, message: r.data?.message };
}
...
export async function awardDeenPoints(
  activity: string,
): Promise<{ ok: boolean; awarded?: number; balance?: number }> {
  if (FORCE_DEMO) return { ok: true, awarded: 0 };
  const r = await request<{
    status?: string;
    awarded?: number;
    new_balance?: number;
  }>("/api/deenpoints/award.php", {
    method: "POST",
    body: { activity },
  });
...
export async function registerPushToken(
  ...
  const r = await request<{ status?: string }>(
    "/api/notifications/register_expo.php",
    {
      method: "POST",
...
export async function events(): Promise<EventItem[]> {
  const r = await request<{ status?: string; events?: EventItem[] }>(
    "/api/events/list.php",
  );
...
export async function unreadNotifications(): Promise<number> {
  const r = await request<{ status?: string; unread_count?: number }>(
...
export async function checkEmailVerified(email: string): Promise<boolean> {
  const r = await request<{ verified?: boolean; is_email_verified?: number }>(
    `/api/auth/check_email_verified.php?email=${encodeURIComponent(email)}`,
  );
  return (
    !!r.ok &&
    (r.data.verified === true || Number(r.data.is_email_verified) === 1)
  );
}
...
export async function checkUsernameAvailable(
  ...
  const r = await request<{ available?: boolean; message?: string }>(
  ...
  return {
    available: false,
    message: r.data?.message ?? "Could not check username",
  };
}
...
export async function checkEmailAvailable(
  ...
  return {
    available: false,
    message: r.data?.message ?? "Could not check email",
  };
}
export async function verifyOtp(
  ...
  const r = await request<{
    ...
    expired?: boolean;
    user?: User | null;
  }>("/api/auth/verify_otp.php", { body: { email, code } });
...
export async function requestEmailChange(
  ...
  const r = await request<{ status?: string; message?: string }>(
    "/api/auth/change_email.php",
    { body: { old_email: oldEmail, new_email: newEmail } },
  );
...
export async function appDefaults(): Promise<AppDefaults | null> {
  const r = await request<{
    status?: string;
    goal_sets?: string[][];
    quick_defaults?: string[];
  }>("/api/defaults/get.php");
  if (
    r.ok &&
    (Array.isArray(r.data.goal_sets) || Array.isArray(r.data.quick_defaults))
  )
    return r.data;
  return null;
}

export async function prayerTimesCached(
  locationHash: string,
): Promise<PrayerTimesResponse | null> {
  const r = await request<PrayerTimesResponse>(
    `/api/get_prayer_times.php?locationHash=${encodeURIComponent(locationHash)}`,
  );
...
export type ChatConversation = {
  ...
  conv_status?: "request" | "active" | "declined";
...
export async function chatConversations(): Promise<ChatConversation[] | null> {
  ...
  }>("/api/chat/conversations.php", { auth: true });
  return r.ok && Array.isArray(r.data.conversations)
...
export async function chatMessages(
  ...
  const r = await request<{ status?: string; messages?: ChatMessage[] }>(
    `/api/chat/messages.php?conversation_id=${conversationId}`,
    { auth: true },
  );
...
export async function chatStartDM(userId: number): Promise<number | null> {
  const r = await request<{ status?: string; conversation_id?: number }>(
    "/api/chat/start.php",
    { method: "POST", body: { user_id: userId }, auth: true },
  );
  return r.ok && r.data.conversation_id
    ? (r.data.conversation_id as number)
    : null;
}
...
export async function chatSharedWithMe(
  kind = "reel",
): Promise<ChatSharedRow[]> {
  try {
    const r = await request<{ status?: string; shares?: ChatSharedRow[] }>(
      `/api/chat/shared_with_me.php${kind ? `?kind=${encodeURIComponent(kind)}` : ""}`,
      { auth: true },
    );
...
export async function chatStartDMByUsername(
  username: string,
): Promise<{ cid: number; status?: string; error?: string } | null> {
  ...
  if (r.ok && r.data.conversation_id) {
    return {
      cid: r.data.conversation_id as number,
  ...
  return {
    ...
    error:
      ...
      (r.networkError
        ? "No connection"
        : r.httpStatus
          ? `Server error (HTTP ${r.httpStatus})`
...
export async function chatSend(
  ...
  if (r.ok && r.data.id) {
    return { id: r.data.id as number, created_at: r.data.created_at };
  }
...
export async function chatDelete(
  conversationId: number,
  targetKind: "msg" | "share",
  targetId: number,
): Promise<boolean> {
  const r = await request<{ status?: string; deleted?: boolean }>(
    "/api/chat/delete.php",
    {
      method: "POST",
      body: {
...
export async function chatTyping(
  conversationId: number,
  typing: boolean,
): Promise<void> {
  await request("/api/chat/typing.php", {
    method: "POST",
    body: { conversation_id: conversationId, typing: typing ? 1 : 0 },
    auth: true,
  });
}
...
export async function chatRequestAction(
  conversationId: number,
  action: "accept" | "block" | "report",
): Promise<boolean> {
  const r = await request<{ status?: string; action?: string }>(
    "/api/chat/request_action.php",
    {
      method: "POST",
...
export async function chatSendShare(
  conversationId: number,
  kind: string,
  title: string,
  ...
  const r = await request<{
    ...
    id?: number;
    created_at?: string;
  }>("/api/chat/send_share.php", {
    method: "POST",
    body: { conversation_id: conversationId, kind, title, payload },
    auth: true,
  });
  return r.ok && r.data.id
    ? { id: r.data.id as number, created_at: r.data.created_at }
    : null;
}
...
export async function chatReact(
  ...
  targetId: number,
  emoji: string,
): Promise<{ ok: boolean; emoji: string | null }> {
  const r = await request<{ status?: string; emoji?: string | null }>(
    "/api/chat/react.php",
    {
      method: "POST",
      body: {
  ...
  return {
    ok: r.ok,
    emoji: r.ok && r.data.emoji ? String(r.data.emoji) : null,
  };
}
export async function chatRead(conversationId: number): Promise<void> {
  await request("/api/chat/read.php", {
    method: "POST",
    body: { conversation_id: conversationId },
    auth: true,
  });
}
export async function chatPresence(): Promise<void> {
  await request("/api/chat/presence.php", { method: "POST", auth: true });
}
...
export async function aiCacheLookup(q: string): Promise<string | null> {
  const r = await request<{ hit?: boolean; answer?: string }>(
    `/api/deenai/cache_lookup.php?q=${encodeURIComponent(q)}`,
  );
  return r.ok && r.data.hit && r.data.answer ? String(r.data.answer) : null;
}
export async function aiCacheSave(
  question: string,
  answer: string,
): Promise<void> {
  await request("/api/deenai/cache_save.php", {
    method: "POST",
...
export async function quizBank(): Promise<QuizBankItem[] | null> {
  try {
    const r = await request<{ status?: string; questions?: QuizBankItem[] }>(
      "/api/quiz/bank.php",
      { method: "GET" },
    );
...
export async function deenAiChatServer(
  question: string,
  ...
  if (r.ok && r.data.answer)
    return { ok: true, answer: r.data.answer, model: r.data.model };
  return {
    ok: false,
    error: r.data.message ?? (r.networkError ? "Offline" : "AI unavailable"),
  };
}

/* Slice 7 — admin-managed Prophets stories chapters; null when offline/empty so the bundled files stay. */
export type AdminProphetChapter = {
  slug: string;
  name: string;
  n: number;
  source: string;
...
export async function prophetChapters(): Promise<AdminProphetChapter[] | null> {
  const r = await request<{
    status?: string;
    chapters?: AdminProphetChapter[];
  }>("/api/prophets/list.php");
  if (r.ok && Array.isArray(r.data.chapters) && r.data.chapters.length > 0)
    return r.data.chapters;
  return null;
}
export async function prophetFull(
  slug: string,
): Promise<AdminProphetChapter | null> {
  const r = await request<{ status?: string; chapter?: AdminProphetChapter }>(
    `/api/prophets/get.php?slug=${encodeURIComponent(slug)}`,
  );
...
export async function namesOfAllah(): Promise<AdminName[] | null> {
  const r = await request<{ status?: string; names?: AdminName[] }>(
    "/api/names/list.php",
  );
  if (r.ok && Array.isArray(r.data.names) && r.data.names.length > 0)
    return r.data.names;
  return null;
}

/* Slice 5 — admin-managed Duas & Athkar additions; null when offline/empty so the bundled set stays. */
export type AdminAthkar = {
  group: string;
  name: string;
  arabic: string;
  transliteration: string;
