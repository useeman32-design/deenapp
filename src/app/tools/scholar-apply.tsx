...
export default function ScholarApplyScreen() {
  ...
  const loadMine = useCallback(async () => {
    ...
    if (r.scholar) {
      ...
      const arr = Array.isArray(raw) ? (raw as unknown[]).map(String) : typeof raw === 'string' ? JSON.parse(raw || '[]') : [];
      if (Array.isArray(arr)) { setFields(arr.filter((x) => FIELDS.includes(String(x))).map(String)); setFieldsOther(arr.filter((x) => !FIELDS.includes(String(x))).join(', ')); }
    }
