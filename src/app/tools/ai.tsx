          ...
          {thinkingHere ? (
            <View style={{ paddingVertical: 3 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 9 }}>
                ...
                <T v="caption" style={{ fontSize: 11, color: d.faint }}>{m.reasoning ? 'Thinking…' : researching(m.text) ? 'reasoning…' : phase === 'retrieving' ? 'Searching your library…' : 'Thinking…'}</T>
              </View>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: d.bg }}>
      ...
      <View style={{ paddingTop: insets.top + 10, paddingHorizontal: 12, paddingBottom: 10, borderBottomWidth: 1, borderBottomColor: glass.border, backgroundColor: glass.bg, flexDirection: 'row', alignItems: 'center', gap: 9 }}>
        ...
        <Pressable accessibilityLabel="Go back" onPress={() => { haptic.selection(); if (router.canGoBack()) goBack(router); }} style={{ width: 36, height: 36, borderRadius: 12, borderWidth: 1, borderColor: glass.border, backgroundColor: isDark ? 'rgba(46,204,113,0.10)' : 'rgba(29,111,66,0.06)', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <View style={{ borderTopWidth: 1, borderTopColor: glass.border, paddingTop: 10, paddingHorizontal: 12, paddingBottom: insets.bottom + 12 }}>
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              <T v="caption" style={{ fontSize: 9, color: d.faint, marginTop: 1 }}>{apiKey ? `${modelList.find((m) => m.id === model)?.note ?? 'deep reasoning'} · web ${webOn ? 'on' : 'off'}` : serverAi ? 'Secure DeenLink AI' : 'Add an API key for full AI'}</T>
            </View>
      ...
      <Modal visible={showSettings} animationType="slide" transparent onRequestClose={() => setShowSettings(false)}>
        <Pressable style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.45)' }} onPress={() => setShowSettings(false)}>
          <Pressable onPress={(e) => stopBubble(e)} style={{ marginTop: insets.top + 60, marginHorizontal: 10, borderRadius: 20, backgroundColor: d.card, borderWidth: 1, borderColor: d.cardBorder, padding: 16 }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, borderRadius: 13, borderWidth: 1, borderColor: 'rgba(46,204,113,0.4)', backgroundColor: isDark ? 'rgba(46,204,113,0.08)' : 'rgba(29,111,66,0.06)', paddingHorizontal: 12, paddingVertical: 12, marginBottom: 14 }}><FontAwesome5 name="shield-alt" size={13} color={isDark ? '#4AE38F' : '#1D6F42'} /><View style={{ flex: 1 }}><T v="bodyS" style={{ fontSize: 12, fontWeight: '800', color: d.text }}>Secure DeenLink AI</T><T v="caption" style={{ fontSize: 9.5, color: d.subtext, marginTop: 2 }}>Provider keys and usage are managed securely by DeenLink administrators.</T></View></View>
