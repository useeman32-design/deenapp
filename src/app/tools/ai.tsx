...
function isImportantMessage(q: string): boolean {
  ...
  return /\b(my name is|i am|i'm|im |call me|remember|i live|i reside|i am from|i'm from|my madhab|i follow|my email|my number|my birthday|i prefer|i like|my family|my wife|my husband|my son|my daughter)\b/.test(
    t,
  );
}
...
const SUGGESTIONS = [
  ...
  { icon: "hands-helping", color: "#7C5CBF", q: "Give me a dua for guidance" },
  {
    icon: "compass",
    color: "#2C6E8F",
    q: "Where can I find the qibla compass?",
  },
];
...
const REF_RE =
  /\[(Quran\s*\d{1,3}:\d{1,3}(?:\s*[-–]\s*\d{1,3})?|[A-Za-z][^\]]*·[^\]]*|web)\]/g;
...
const HADITH_ROUTES: Array<[RegExp, string]> = [
  [/bukhari/i, "/tools/hadith/bukhari"],
  [/muslim/i, "/tools/hadith/muslim"],
  [/abu\s*dawud|abudawud/i, "/tools/hadith/abudawud"],
  [/nawawi/i, "/tools/hadith/nawawi40"],
  [/shamail/i, "/tools/hadith/shamail_muhammadiyah"],
  [/riyad/i, "/tools/hadith/riyad_assalihin"],
  [/malik/i, "/tools/hadith/malik"],
];
  ...
  chunks.forEach((c, ci) => {
      ...
      out.push(
        <T
          ...
          style={{
            ...
            fontWeight: "900",
            color: "#E8C96A",
            textDecorationLine: refRoute(c.s) ? "underline" : "none",
          }}
        >
          [{c.s}]
        </T>,
    ...
    if (bareRefs.length) {
      ...
      for (const br of bareRefs) {
        if (br.i > lastIx)
          out.push(bodyTxt.slice(lastIx, br.i).replace(/\*{1,2}/g, ""));
        out.push(
          <T
            key={`${key}-${ci}-r${tk++}`}
            v="bodyS"
            onPress={() => {
              haptic.selection();
              onNav(
                `/read/${br.ref.split(" ")[1].split(":")[0]}?ayah=${br.ref.split(":")[1]}`,
              );
            }}
            style={{
              fontSize: 13.5,
      ...
      if (lastIx < bodyTxt.length)
        out.push(bodyTxt.slice(lastIx).replace(/\*{1,2}/g, ""));
      return;
    }
...
function parseBlocks(text: string): Block[] {
  ...
  for (const raw of text.split("\n")) {
    ...
    if (h) {
      blocks.push({ k: "h", s: h[1].replace(/[ *#]+$/, "") });
      continue;
    }
    const b = line.match(/^\s*[-*•]\s+(.*)$/);
    if (b) {
      blocks.push({ k: "b", marker: "•", s: b[1] });
      continue;
    }
    const n = line.match(/^\s*(\d{1,2})[.)]\s+(.*)$/);
    if (n) {
      blocks.push({ k: "b", marker: `${n[1]}.`, s: n[2] });
      continue;
    }
      ...
      {blocks.map((bl, i) => {
        ...
        if (bl.k === "b") {
          return (
            <View
              key={i}
              style={{ flexDirection: "row", gap: 7, marginTop: 3 }}
            >
              <T
                v="bodyS"
                style={{
...
function VerseCards({
  ...
  useEffect(() => {
    ...
    for (const m of cleanAIStatic(text).matchAll(
      /\[Quran\s*(\d{1,3}):(\d{1,3})/g,
    )) {
      const key = `${m[1]}:${m[2]}`;
        ...
        try {
          ...
          if (v)
            out.push({
              ...
              english: v.english,
              name:
                QURAN.find((q) => q.number === r.surah)?.english ??
                `Surah ${r.surah}`,
            });
        } catch {}
      }
    ...
    <View>
      {cards.map((c, i) => (
        <Pressable
          ...
          onPress={() => {
            haptic.selection();
            onOpen(`/read/${c.surah}?ayah=${c.ayah}`);
          }}
          style={{
            marginTop: 9,
            borderRadius: 13,
            borderWidth: 1,
            borderColor: "rgba(29,111,66,0.28)",
            backgroundColor: isDarkStatic
        ...
        >
          ...
          >
            ...
            <T
              v="caption"
              style={{ fontSize: 9.5, fontWeight: "800", color: "#B8870B" }}
            >
              {c.name} {c.surah}:{c.ayah} · tap to open
            </T>
          </View>
        </Pressable>
      ))}
...
function FeedbackRow({ text, msgKey }: { text: string; msgKey: number }) {
  ...
  const chip = (
    ...
    <Pressable
      ...
      style={{
        ...
        borderRadius: 999,
        borderWidth: 1,
        borderColor: active ? `${tint}66` : "rgba(140,150,145,0.25)",
    ...
    >
      <FontAwesome5
        name={icon}
        size={9}
        color={active ? tint : "rgba(140,150,145,0.7)"}
        solid={icon !== "copy"}
      />
      <T
        v="caption"
        style={{
          fontSize: 9.5,
          fontWeight: "700",
          color: active ? tint : "rgba(140,150,145,0.8)",
        }}
      >
        {label}
      </T>
  ...
  return (
    ...
    >
      {chip("thumbs-up", "Helpful", vote === "up", "#1F8F5C", () =>
        setVote((v) => (v === "up" ? null : "up")),
      )}
      {chip("thumbs-down", "Not helpful", vote === "down", "#C0392B", () =>
...
export default function DeenLinkAI() {
  ...
  useEffect(() => {
    storage.getItem(MEM_KEY).then((r) => {
      try {
        const a = JSON.parse(r ?? "[]");
        if (Array.isArray(a)) setMemories(a);
      } catch {
        /* ignore */
      }
    });
  }, []);
  const persistMem = useCallback((next: Memory[]) => {
    setMemories(next);
    storage.setItem(MEM_KEY, JSON.stringify(next)).catch(() => {});
  }, []);
  isDarkStatic = isDark;
  ...
  const [phase, setPhase] = useState<
    "idle" | "retrieving" | "thinking" | "streaming"
  >("idle");
  ...
  const patchChat = useCallback(
    (chatId: string, fn: (m: AiMsg[]) => AiMsg[]) => {
      setChats((prev) => {
  ...
  useEffect(() => {
    ...
    if (isLive()) {
      aiServerStatus()
        .then((s) => setServerAi(s.connected))
        .catch(() => setServerAi(false));
    } else {
      setServerAi(false);
    }
  ...
  const send = async (text: string) => {
    ...
    setChats((prev) =>
      prev.some((c) => c.id === chatId)
        ? prev
        : [
            { id: chatId, title: q.slice(0, 42), at: Date.now(), msgs: [] },
            ...prev,
          ],
    );
    ...
    if (isImportantMessage(q)) {
      ...
      if (!memories.some((m) => m.text.toLowerCase() === t.toLowerCase())) {
        persistMem(
          [{ id: uid(), text: t, at: Date.now() }, ...memories].slice(0, 50),
        );
        savedMem = true;
      }
    }
    ...
    if (!apiKey && nav) {
      ...
      patchChat(chatId, (m) => [
        ...m,
        { role: "assistant", text: nav.text, at: Date.now(), nav: nav.route },
      ]);
    ...
    if (!apiKey && isGreeting(q)) {
      patchChat(chatId, (m) => [
        ...
        {
          role: "assistant",
          text: greetingAnswer(q, firstName),
          at: Date.now(),
        },
      ]);
      setPhase("idle");
    ...
    try {
      sources = await retrieveLocal(q);
    } catch {}
    patchChat(chatId, (m) => [
      ...m,
      { role: "assistant", text: "", at: Date.now(), memSaved: savedMem },
    ]);
    ...
    if (apiKey) {
      ...
      if (cached) {
        patchChat(chatId, (m) => {
          const c = [...m];
          c[c.length - 1] = { ...c[c.length - 1], text: cached };
          return c;
        });
        setBusyIds((b) => b.filter((x) => x !== chatId));
        setPhase("idle");
      ...
      setPhase("thinking");
      const history = [...msgs.filter((m) => m.text), userMsg]
        .slice(-8)
        .map(
          (m) =>
            ({ role: m.role, content: m.text }) as {
              role: "user" | "assistant";
              content: string;
            },
        );
      ...
      await streamLLM(
        ...
        (e) => {
          ...
          if (e.citations?.length) {
            const webSrcs: AiSource[] = e.citations.map((c) => ({
              kind: "web" as const,
              label: shortUrl(c),
              excerpt: c,
            }));
            patchChat(chatId, (m) => {
              ...
              c[c.length - 1] = {
                ...c[c.length - 1],
                sources: [...(c[c.length - 1].sources ?? []), ...webSrcs],
              };
              return c;
            });
          }
      ...
      if (err) {
        ...
        patchChat(chatId, (m) => {
          ...
          c[c.length - 1] = {
            ...c[c.length - 1],
            text: `⚠️ The model returned an empty response (the free tier may be rate-limited — try again in a minute).\n\n${fallback}`,
          };
          return c;
        });
      } else if (navRoute) {
        patchChat(chatId, (m) => {
          const c = [...m];
          c[c.length - 1] = { ...c[c.length - 1], nav: navRoute };
          return c;
        });
      }
      ...
      patchChat(chatId, (m) => {
        ...
        c[c.length - 1] = {
          ...c[c.length - 1],
          sources: [...(c[c.length - 1].sources ?? []), ...cited],
        };
        return c;
      });
    } else {
      ...
      try {
        ...
        const serverMessages = [
          { role: "system", content: sys } as { role: string; content: string },
          ...history,
        ];
      ...
      patchChat(chatId, (m) => {
        ...
        c[c.length - 1] = {
          ...c[c.length - 1],
          text: composeLocalAnswer(q, sources),
          sources,
        };
        return c;
      });
  ...
  const openChat = (c: AiChat) => {
    ...
    setMsgs(c.msgs);
    setDrawerOpen(false);
    setTimeout(() => scroller.current?.scrollToEnd({ animated: false }), 120);
  };
  ...
  const glass = isDark
    ? { bg: "rgba(18,34,25,0.72)", border: "rgba(74,227,143,0.25)" }
    : { bg: "rgba(255,255,255,0.78)", border: "rgba(29,111,66,0.18)" };
  const cleanAI = (t: string) =>
    t
      ...
      .replace(/<think>[\s\S]*$/, "")
      .replace(/<tool>[\s\S]*$/, "");
  const researching = (t: string) =>
    cleanAI(t).trim().length === 0 && t.trim().length > 0;
  ...
  const bubble = (m: AiMsg, i: number) => {
    ...
    const thinkingHere =
      ...
      busy &&
      i === msgs.length - 1 &&
      (m.text.trim() === "" || researching(m.text));
    const body = cleanAI(m.text)
      .replace(NAV_LINE, "")
      .replace(NAV_STRIP, "")
      .replace(/\n{3,}/g, "\n\n")
      .trim();
      ...
      >
        {!mine ? (
          ...
          >
            ...
            >
              DEENLINK{" "}
              {apiKey && provider
                ? `· ${modelList.find((m) => m.id === model)?.note ?? "deep reasoning"}`
                : "· ON-DEVICE"}
            </T>
            {m.thinkMs != null && m.thinkMs > 800 && m.text ? (
              ...
              >
                ...
                <T
                  v="caption"
                  style={{ fontSize: 8, fontWeight: "700", color: d.faint }}
                >
                  {(m.thinkMs / 1000).toFixed(1)}s
                </T>
              </View>
            ) : null}
          </View>
        ...
        <View
          style={{
            ...
            paddingVertical: 10,
            backgroundColor: mine ? (isDark ? "#1F8F5C" : "#1D6F42") : glass.bg,
          ...
          {thinkingHere ? (
            ...
            <>
              <AnswerText
                ...
                accent={isDark ? "#4AE38F" : "#1D6F42"}
                onNav={(rt) => router.push(rt as never)}
              />
              {!mine ? (
                <VerseCards
                  text={body}
                  onOpen={(rt) => router.push(rt as never)}
                />
              ) : null}
            </>
          )}
        ...
        {!mine && nav && !thinkingHere ? (
          ...
          >
            ...
            <T
              v="caption"
              style={{ fontSize: 10.5, fontWeight: "800", color: "#fff" }}
            >
              Open{" "}
              {NAV_LABELS[nav] ?? nav.replace("/tools/", "").replace("/", " ")}
            </T>
          </Pressable>
        ) : null}
            ...
            {dedupeSources(m.sources)
              ...
              .map((s, j) => (
                <Pressable
                  ...
                  onPress={() => {
                    haptic.selection();
                    if (s.href) router.push(s.href as never);
                  }}
                  style={{
                    flexDirection: "row",
                    alignItems: "center",
                    ...
                    borderWidth: 1,
                    borderColor:
                      s.kind === "web" ? "rgba(44,110,143,0.4)" : glass.border,
                    backgroundColor:
                      s.kind === "web"
    ...
    <View style={{ flex: 1, backgroundColor: d.bg }}>
      ...
      >
        ...
        <Pressable
          ...
          onPress={() => {
            haptic.selection();
            if (router.canGoBack()) goBack(router);
          }}
          style={{
            width: 36,
            height: 36,
      ...
      <Animated.View
        style={{
          ...
          transform: [
            {
              translateX: drawerX.interpolate({
                inputRange: [0, 1],
                outputRange: [-312, 0],
              }),
            },
      ...
      >
            ...
            chats.map((c) => (
              <Pressable
                ...
                style={{
                  ...
                  borderWidth: 1,
                  borderColor:
                    activeId === c.id ? "rgba(31,143,92,0.45)" : glass.border,
                  backgroundColor:
                    activeId === c.id
                      ? isDark
                        ? "rgba(46,204,113,0.10)"
              ...
              >
                ...
                <View style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    {timeAgo(c.at)} · {c.msgs.length} msgs
                  </T>
                </View>
                <Pressable
                  hitSlop={8}
        ...
        {chats.length ? (
          ...
          >
            <T
              v="caption"
              style={{ fontSize: 10.5, fontWeight: "800", color: "#DC5050" }}
            >
              Clear all history
            </T>
          </Pressable>
            ...
            <View>
              ...
              >
                ...
                >
                  ...
                  <ExpoImage
                    source={require("../../../assets/img/logo-export.png")}
                ...
                >
                  Ask about the Qur{"\u2019"}an, hadith & duas — or where
                  anything lives in the app. I answer from your library
                  {apiKey ? " + AI reasoning" : ""}.
                </T>
              </View>
              ...
              {(
                ...
                >
                  <FontAwesome5
                    name={o.icon}
                    size={12}
                    color={webToggle === o.k ? "#4AE38F" : d.faint}
                  />
                  <View style={{ flex: 1 }}>
                    <T
                      v="bodyS"
                    ...
                    <T v="caption" style={{ fontSize: 9, color: d.faint }}>
                      {o.note}
                    </T>
                  </View>
                  {webToggle === o.k ? (
                    <FontAwesome5 name="check" size={11} color="#4AE38F" />
                  ) : null}
                </Pressable>
              ))}
            </View>
          ...
          >
            <Pressable
              ...
              style={{
                ...
                borderRadius: 16,
                borderWidth: 1,
                borderColor: webToggle ? "rgba(212,175,55,0.6)" : glass.border,
                backgroundColor: webToggle
            ...
            >
              <FontAwesome5
                name="lightbulb"
                size={14}
                color={webToggle ? "#D4AF37" : d.faint}
              />
            </Pressable>
            <View
              style={{
            ...
            <Pressable
              ...
              onPress={() => {
                if (busy) {
                  haptic.light();
                  activeId != null && aborts.current.get(activeId)?.abort();
                } else send(draft);
              }}
            ...
            >
              {busy ? (
                <ActivityIndicator size="small" color="#DC5050" />
              ) : (
                <FontAwesome5
                  name="paper-plane"
                  size={15}
                  color={!draft.trim() ? d.faint : "#FFFFFF"}
                />
              )}
            </Pressable>
          </View>
          ...
          >
            ...
            <View style={{ marginTop: 16 }}>
              ...
              {memories.length === 0 ? (
                <T
                  v="caption"
                  style={{ fontSize: 10.5, color: d.faint, lineHeight: 16 }}
                >
                  Nothing saved yet. When you share something personal — your
                  name, madhab, preferences — I remember it here so future
                  answers fit you. You can delete any item.
                </T>
              ) : (
                <View style={{ gap: 7 }}>
                  {memories.map((m) => (
                    ...
                    >
                      <T
                        v="bodyS"
                        style={{ flex: 1, fontSize: 11.5, color: d.text }}
                      >
                        {m.text}
                      </T>
                      <Pressable
                        onPress={() => {
                          haptic.selection();
                          persistMem(memories.filter((x) => x.id !== m.id));
                        }}
                        hitSlop={6}
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <T
                  v="caption"
                  style={{ fontSize: 9.5, color: d.subtext, marginTop: 2 }}
                >
                  Provider keys and usage are managed securely by DeenLink
                  administrators.
                </T>
              </View>
            </View>
            ...
            >
              Choose the AI reasoning capability. Provider keys, usage logging,
              and web access are managed securely by DeenLink administrators.
            </T>
          </Pressable>
        </Pressable>
      </Modal>
...
const dedupeSources = (list: AiSource[]) => {
  const seen = new Set<string>();
