 ...
 *  · metal prices are LIVE from IslamicAPI (read-only — realtime, not editable);
      ...
      <ScrollView contentContainerStyle={{ paddingTop: insets.top + 14, paddingBottom: 40 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 18 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5, borderRadius: 8, borderWidth: 1, borderColor: live === 'live' ? 'rgba(74,227,143,0.4)' : d.cardBorder, backgroundColor: live === 'live' ? (isDark ? 'rgba(46,204,113,0.08)' : 'rgba(29,111,66,0.05)') : 'transparent', paddingHorizontal: 7, paddingVertical: 3 }}>
            ...
            <T v="caption" style={{ fontSize: 8.5, fontWeight: '800', letterSpacing: 0.4, color: live === 'live' ? (isDark ? '#4AE38F' : '#1D6F42') : d.faint }}>
              {live === 'live' ? 'LIVE PRICES · ISLAMICAPI' : live === 'loading' ? 'FETCHING…' : 'OFFLINE PRICES'}
            </T>
          </View>
        </View>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginHorizontal: 16, marginBottom: 8 }}>
          {(['gold', 'silver'] as const).map((s) => {
            const on = standard === s;
            return (
              <Pressable key={s} onPress={() => { haptic.selection(); setStandard(s); }} accessibilityLabel={`nisab standard ${s}`} style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 10, borderRadius: 12, borderWidth: 1, borderColor: on ? (isDark ? 'rgba(212,175,55,0.55)' : 'rgba(184,134,11,0.5)') : d.cardBorder, backgroundColor: on ? 'rgba(212,175,55,0.1)' : d.card }}>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginHorizontal: 16, marginBottom: 8 }}>
          {([
            ...
            <View key={m.k} style={{ flex: 1, borderRadius: 14, borderWidth: 1, borderColor: d.cardBorder, backgroundColor: d.card, paddingHorizontal: 12, paddingVertical: 8 }}>
              ...
              {/* pass 39 — you enter GRAMS; the realtime price is never editable */}
              ...
              <T v="caption" style={{ fontSize: 9.5, color: d.faint, marginTop: 4 }}>= <T v="caption" style={{ fontSize: 10, fontWeight: '800', color: isDark ? '#4AE38F' : '#1D6F42' }}>{money(m.value)}</T> live valuation</T>
            </View>
          ))}
        </View>
        ...
        >
          <FontAwesome5 name="calculator" size={14} color="#fff" />
          <T v="button" style={{ color: '#FFFFFF', fontWeight: '800', fontSize: 14 }}>Calculate zakat</T>
        </Pressable>
