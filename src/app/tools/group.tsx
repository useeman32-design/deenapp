...
function BreathingPosts({
  dash,
}: {
  dash: { card: string; cardBorder: string };
}) {
          ...
          <View style={{ flexDirection: "row", alignItems: "center", gap: 9 }}>
            <View
              style={{
                ...
                height: 34,
                borderRadius: 17,
                backgroundColor: dash.cardBorder,
              }}
            />
...
function GroupScreenInner() {
  ...
  const id =
    typeof params.id === "string" ? params.id : String(params.id ?? "g1");
  ...
  const [imagesAttach, setImagesAttach] = useState<
  ...
  useEffect(() => {
    publicSettings()
      .then((fl: Record<string, boolean | string>) =>
        setVideoAllowed(fl["posting.community_video"] === true),
      )
      ...
      if (!res.canceled && res.assets?.length) {
        ...
        setImagesAttach((cur) =>
          [
            ...
            ...res.assets
              .filter((a) => a.uri)
              .map((a) => ({ uri: a.uri, name: a.fileName ?? "photo.jpg" })),
          ].slice(0, 5),
        );
      }
    } catch {
  ...
  const pickAudio = async () => {
    ...
    try {
      ...
      /* picker unavailable — nothing attached */
    }
  };
  ...
  const validateVideo = (name: string, size?: number): string | null => {
    if (!["mp4", "mov", "webm", "m4v"].includes(extOf(name)))
      return `“${name}” is not a supported video file (mp4, mov, webm, m4v).`;
    if (size != null && size > 50 * 1024 * 1024)
      return `“${name}” is over the 50 MB video limit.`;
    return null;
  };
  const YT_RE =
  ...
  const pickVideo = async () => {
    ...
    try {
      ...
      if (!res.canceled && a?.uri) {
        const err = validateVideo(
          a.fileName ?? "video.mp4",
          a.fileSize ?? undefined,
        );
        if (err) {
          setPostError(err);
          return;
  ...
  const [roster, setRoster] = useState<NonNullable<GroupRow["members"]> | null>(
    null,
  );
  ...
  const [joinReqs, setJoinReqs] = useState<Array<{
    id: number;
    username: string;
    full_name: string;
  ...
  const loadServerPosts = (
    ...
    void groupPostsApi(sid)
      .then((rows) => {
        if (!rows) {
          if (!retried) {
            setTimeout(() => loadServerPosts(sid, localId, true), 900);
            return;
          }
          setPostsError(true);
          return;
        ...
        setGroup((cur) =>
          cur && (lid == null ? true : cur.id === lid)
            ? {
                ...
                posts: rows.map((p) => ({
                  id: `sp${p.id}`,
                  author: p.user?.full_name || p.user?.username || "Member",
                  text: p.content_text ?? "",
                  at: new Date(p.created_at ?? Date.now()).getTime(),
                })),
              }
            : cur,
        );
  ...
  const join = () => {
    ...
    if (sid != null) {
      ...
      upd((x) => ({
        ...x,
        joined: "member",
        members: x.members.includes(ME) ? x.members : [...x.members, ME],
      ...
      void groupJoinRich(sid, true).then((res) => {
        ...
        if (ok) {
          ...
          upd((x) => ({
            ...x,
            joined: null,
            members: x.members.filter((m) => m !== ME),
    ...
    if (sid != null) {
      ...
      groupJoinRich(sid, true).then((res) => {
        ...
        if (res === "requested") {
          Alert.alert(
            "Request sent",
            "The group admin will review your join request.",
          );
        } else if (res === "joined") {
          void groupGet(sid).then((row) => {
            if (!row) return;
    ...
    upd((x) => ({
      ...x,
      joined: x.open ? "member" : "requested",
      members: x.open
        ? x.members.includes(ME)
          ? x.members
          : [...x.members, ME]
        : x.members,
      memberCount: x.open ? x.memberCount + 1 : x.memberCount,
      ...(x.open ? { my_role: "member" as Role } : {}),
    }));
  };
  const leave = () => {
    if (!group || joinBusy) return;
  ...
  const post = () => {
    ...
    if (yt && !YT_RE.test(yt)) {
      setPostError("That does not look like a YouTube link.");
      return;
    }
    if (
      (!composer.trim() &&
    ...
    upd((x) => ({
      ...x,
      posts: [
        { id: `p${Date.now()}`, author: ME, text, at: Date.now() },
        ...x.posts,
      ],
    })); /* pass 83-35 — no local media preview in the list */
    ...
    if (serverPosts) {
      const optimistic = {
        ...
        ...(pollOk
          ? {
              poll: {
                options: poll.map((o, i) => ({ id: i + 1, text: o, votes: 0 })),
                voted: null,
              },
            }
          : {}),
    ...
    if (sid != null) {
      ...
      const verifyMaybePosted = async (): Promise<boolean> => {
        try {
          ...
          const hasMedia =
            Array.isArray((newest as { media?: unknown[] }).media) &&
            ((newest as { media?: unknown[] }).media ?? []).length > 0;
          const ts = Date.parse(
            String(newest.created_at ?? "").replace(" ", "T") + "Z",
          );
          const fresh = Number.isNaN(ts)
            ? true
            : Date.now() - ts < 6 * 3600 * 1000;
      ...
      void groupCreatePost(
        ...
        text,
        imgs.length
          ? imgs.map((m) => ({ uri: m.uri, name: m.name, type: "image/jpeg" }))
          : undefined,
        ...
        .then(async (res) => {
          ...
          if (res && res.id != null) {
            ...
            setServerPosts((rows) =>
              (rows ?? []).filter((r) => r.id !== optId),
            );
            restoreGroupDraft(draft);
            setPostError(
              res?.message
                ? `Unable to post — ${res.message}`
  ...
  const canSend =
    ...
    (!!composer.trim() ||
      ...
      !!videoAttach ||
      ytReady ||
      (pollOn && pollOpts.filter((o) => o.trim()).length >= 2));

  /* pass 83-32 — put a failed post's content back into the composer */
  const restoreGroupDraft = (d: {
    text: string;
    imgs: typeof imagesAttach;
    vid: typeof videoAttach;
    aud: typeof audioAttach;
    yt: string;
    pollOn: boolean;
    poll: string[];
    ytOn: boolean;
  ...
  const removeMember = (member: string) => {
    ...
    upd((x) => {
      ...
      return {
        ...x,
        members: x.members.filter((m) => m !== member),
        roles,
        memberCount: Math.max(0, x.memberCount - 1),
      };
    });
  ...
  const addMember = (name: string) => {
    haptic.success();
    upd((x) =>
      x.members.includes(name)
        ? x
        : {
            ...x,
            members: [...x.members, name],
            roles: { ...(x.roles ?? {}), [name]: "member" },
            memberCount: x.memberCount + 1,
          },
    );
  };
  ...
  const refreshRoster = (sid: number) => {
    void groupGet(sid).then((row) => {
      ...
      if (Array.isArray(row.members)) setRoster(row.members);
      setGroup((cur) =>
        cur ? { ...cur, memberCount: row.member_count } : cur,
      );
    });
  };
  ...
  const [addDenials, setAddDenials] = useState<
  ...
  const commitDenial = (d: { username: string; full_name: string }) => {
    const list = addDenialsRef.current.some((x) => x.username === d.username)
      ...
      : [...addDenialsRef.current, d].sort((a, b) =>
    ...
    try {
      ...
      if (sid != null) {
        await shareLink({
          ...
          id: sid,
          title: group.name,
          text: group.bio || group.desc,
        });
      } else {
        await Share.share({
          message: `${group.name} — join my DeenLink group!`,
        });
      }
    } catch {
      /* dismissed */
  ...
  const serverRank = (sp: Post): Role => {
    const ar = String(
      (sp as { author_role?: unknown }).author_role ?? "",
    ).toLowerCase();
      ...
      >
        {loadDone ? (
          ...
          >
            ...
            <T
              v="bodyS"
              style={{ fontSize: 12.5, fontWeight: "700", color: d.subtext }}
            >
              Group not found
            </T>
            <Pressable
            ...
            >
              <T
                v="caption"
                style={{ fontSize: 11, fontWeight: "800", color: d.text }}
              >
                Go back
              </T>
            </Pressable>
  ...
  const coverStyle =
    COVER_STYLES.find((c) => c.id === group.cover) ?? COVER_STYLES[0];
  const isMember =
    group.joined === "member" ||
    roleOf(group, ME) === "owner" ||
    roleOf(group, ME) === "admin";
  const userOf = (name: string) =>
    name === ME
      ? { name: "You", user: "you" }
      : { name, user: name.toLowerCase().replace(/[^a-z]+/g, ".") };
          ...
          borderColor:
            role === "owner"
              ...
              : role === "admin"
                ...
                : d.cardBorder,
          borderRadius: 7,
          paddingHorizontal: big ? 8 : 6,
          paddingVertical: big ? 3 : 2,
          backgroundColor:
            role === "owner"
              ? "rgba(212,175,55,0.1)"
              : role === "admin"
    ...
    <View style={{ flex: 1, backgroundColor: d.bg }}>
      ...
      {uploadFrac != null ? (
        ...
        >
          ...
          >
            Posting… {Math.round(uploadFrac * 100)}%
          </T>
          <View
            style={{
              height: 6,
          ...
          >
            <View
              style={{
                height: 6,
                borderRadius: 3,
                width: `${Math.max(4, Math.round(uploadFrac * 100))}%`,
        ...
        <View>
          ...
          {canManage ? (
            ...
            >
              ...
              <T
                v="caption"
                style={{ fontSize: 9.5, fontWeight: "800", color: "#E8C96A" }}
              >
                Change cover
              </T>
            </Pressable>
        ...
        <View style={{ paddingHorizontal: 16 }}>
          ...
          >
            {isGroupImg(group.avatar) ? (
              ...
              <T v="h1" style={{ fontSize: 32 }}>
                {group.avatar}
              </T>
            ) : (
              <FontAwesome5
                name={catIcon(group.cat)}
                size={24}
                color="#E8C96A"
              />
            )}
          ...
          >
            ...
            >
              <View
                style={{
                  flex: 1,
                  backgroundColor: "rgba(3,7,5,0.62)",
                ...
                >
                  ...
                  <FriendsPicker
                    share={{
                      kind: "group",
                      title: group.name,
                      sub: `${group.memberCount.toLocaleString()} members${group.bio ? ` · ${group.bio.slice(0, 60)}` : ""}`,
                      route: `/tools/group?id=${group.id}`,
                    }}
                    onDone={() => setTimeout(() => setInviteOpen(false), 1400)}
                  />
                </View>
              </View>
            </Modal>
          ...
          <T
            v="caption"
            style={{ fontSize: 11, color: d.subtext, marginTop: 4 }}
          >
            {group.cat} group ·{" "}
            {group.open ? "Anyone can join" : "Join by request"}
          </T>
          ...
          <View style={{ flexDirection: "row", gap: 7, marginTop: 10 }}>
            ...
            >
              ...
              <T
                v="caption"
                style={{ fontSize: 10, fontWeight: "800", color: "#E8C96A" }}
              >
                ...
                backgroundColor: group.joined
                  ...
                  : canManage
                    ...
                    : isDark
                      ? "#2ECC71"
                      : "#1D6F42",
                borderWidth: group.joined || canManage ? 1 : 0,
                borderColor: group.joined
                  ? d.cardBorder
                  : "rgba(212,175,55,0.5)",
                alignItems: "center",
                justifyContent: "center",
            ...
            >
              {joinBusy ? (
                <ActivityIndicator
                  size="small"
                  color={
                    group.joined ? d.subtext : canManage ? "#E8C96A" : "#fff"
                  }
                />
              ) : (
                <T
        ...
        >
          {[
              ...
              : []),
            {
              ...
              label: "Members",
              icon: "users",
              n: roster?.length ?? group.members.length,
            },
          ...
          <View style={{ paddingHorizontal: 16, paddingTop: 14, gap: 12 }}>
            ...
            {postsError && serverPosts == null ? (
              ...
              >
                ...
                <Pressable
                  onPress={() => {
                    const sid = group ? srvGroupId(group) : null;
                    if (sid != null) loadServerPosts(sid);
                  }}
                  style={{
                    borderRadius: 10,
                    backgroundColor: isDark ? "#2ECC71" : "#1D6F42",
                ...
                >
                  <Pressable
                    ...
                    style={{
                      ...
                      height: 40,
                      borderRadius: 12,
                      backgroundColor: imagesAttach.length
                        ? isDark
                  ...
                  >
                    <FontAwesome5
                      ...
                      size={14}
                      color={
                        pollOn ? (isDark ? "#4AE38F" : "#0E7A46") : d.faint
                      }
                    />
                  </Pressable>
                  ...
                  >
                    <FontAwesome5
                      ...
                      size={13}
                      color={
                        audioAttach ? (isDark ? "#4AE38F" : "#0E7A46") : d.faint
                      }
                    />
                  </Pressable>
                  ...
                  >
                    <FontAwesome5
                      name="youtube"
                      size={14}
                      color={ytOn ? (isDark ? "#4AE38F" : "#0E7A46") : d.faint}
                    />
                  </Pressable>
                  <View style={{ flex: 1 }} />
                  <Pressable
                        ...
                        {pollOpts.length > 2 ? (
                          <Pressable
                            ...
                            style={{
                              ...
                              height: 24,
                              borderRadius: 12,
                              backgroundColor: "rgba(207,58,58,0.10)",
                              alignItems: "center",
                              justifyContent: "center",
                            }}
                          >
            ...
            {(
              ...
              serverPosts.map((sp) => (
                <FeedCard
                  ...
                  onDelete={
                    (sp.user?.id != null &&
                      user?.id != null &&
                      String(sp.user.id) === String(user.id)) ||
                    (sp.user?.username &&
                      user?.username &&
                      ...
                      ? () => {
                          void groupDeletePost(sp.id).then((ok) => {
                            if (ok) {
                              ...
                              setServerPosts((rows) =>
                                (rows ?? []).filter((x) => x.id !== sp.id),
                              );
                              setGroup((cur) =>
                                cur
                                  ? {
                                      ...cur,
                                      posts: cur.posts.filter(
                                        (x) => x.id !== `sp${sp.id}`,
                                      ),
                                    }
                                  : cur,
              ...
              feedPosts.map((p) => {
                ...
                const fp = {
                  id: Math.abs(
                    [...p.id].reduce((a, c) => a + c.charCodeAt(0), 0) +
                      (p.at % 100000),
                  ),
                  content_text: p.text,
        ...
        {tab === "members" ? (
          <View style={{ paddingHorizontal: 16, paddingTop: 14 }}>
            ...
            >
              {(roster
                ? roster.slice(0, 5).map((m) => ({
                    key: `r${m.id}`,
                    label: (m.full_name || m.username || "?").slice(0, 1),
                  }))
                : group.members
                    .slice(0, 5)
                    .map((m, i) => ({ key: `${m}${i}`, label: m.slice(0, 1) }))
              ).map((a, i) => (
                <View
                  key={a.key}
                  style={{
              ...
              >
                {group.memberCount.toLocaleString()} people are in this group
              </T>
              {canManage ? (
                <Pressable
            ...
            {roster
              ? roster.map((rm) => {
                  const rRole = (
                    ["owner", "admin", "member"].includes(String(rm.role))
                      ? rm.role
                      : "member"
                  ) as Role;
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {rm.profile_image_url ? (
                            <ExpoImage
                              source={{ uri: rm.profile_image_url }}
                              style={{ width: "100%", height: "100%" }}
                              contentFit="cover"
                            />
                          ) : (
                      ...
                      {roleMenu === menuKey ? (
                        ...
                        >
                          {isOwner ? (
                            <Pressable
                              ...
                              style={{
                                ...
                                borderRadius: 10,
                                borderWidth: 1,
                                borderColor: "rgba(47,164,107,0.4)",
                                backgroundColor: "rgba(47,164,107,0.07)",
                                paddingHorizontal: 11,
                                paddingVertical: 9,
                                opacity: busy ? 0.5 : 1,
                              }}
                          ...
                          <Pressable
                            onPress={() => removeServerMember(rm.id)}
                            disabled={busy}
                            style={{
                              flexDirection: "row",
                              alignItems: "center",
                    ...
                    >
                      ...
                      >
                        ...
                        {m !== ME ? (
                          <Pressable
                            accessibilityLabel={
                              isFollowing ? `unfollow ${m}` : `follow ${m}`
                            }
                        ...
                        >
                          {isOwner ? (
                            ...
                            >
                              <FontAwesome5
                                name={
                                  role === "admin" ? "arrow-down" : "shield-alt"
                                }
                                size={11}
                                color="#2FA46B"
                              />
        ...
        {tab === "about" ? (
          <View style={{ paddingHorizontal: 16, paddingTop: 14, gap: 10 }}>
            ...
            >
              ...
              <T
                v="bodyS"
                style={{ fontSize: 12.5, lineHeight: 19, color: d.subtext }}
              >
                {group.desc}
              </T>
            </View>
            {[
              ...
              {
                icon: group.open ? "lock-open" : "lock",
                label: "Privacy",
                value: group.open
                  ? "Open — anyone can join"
                  : "Private — join by request",
              },
              {
                icon: "users",
                label: "Members",
              ...
              },
              {
                icon: "file-alt",
                label: "Posts",
                value: String(group.posts.length),
              },
              {
                icon: "crown",
                label: "Owner",
                value:
                  group.members.find((m) => roleOf(group, m) === "owner") ??
                  (group.mine ? ME : "—"),
              },
            ].map((r) => (
              <View
              ...
              >
                <View
                  style={{
                    ...
                    height: 34,
                    borderRadius: 11,
                    backgroundColor: "rgba(91,200,245,0.1)",
                    alignItems: "center",
                    justifyContent: "center",
                  }}
                >
                ...
                <T
                  v="caption"
                  style={{ fontSize: 10, color: d.faint, width: 74 }}
                >
                  {r.label}
                </T>
                <T
      ...
      >
        <View
          style={{
            flex: 1,
            backgroundColor: "rgba(3,7,5,0.55)",
            justifyContent: "flex-end",
          }}
        >
            ...
            >
              ...
              {srvGroupId(group) != null ? (
                <>
                  <T
                    v="caption"
                    style={{ fontSize: 10, color: d.faint, marginBottom: 10 }}
                  >
                    Search DeenLink accounts by name or username
                  </T>
                  <View
                    style={{
                      flexDirection: "row",
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {a.profile_image_url ? (
                            <ExpoImage
                              source={{ uri: a.profile_image_url }}
                              style={{ width: "100%", height: "100%" }}
                              contentFit="cover"
                            />
                          ) : (
                            ...
                            >
                              {(a.full_name || a.username || "?").slice(0, 1)}
                            </T>
                          )}
                        </View>
                        <View style={{ flex: 1 }}>
                        ...
                        <Pressable
                          onPress={() => addServerMember(a.id)}
                          disabled={inGroup || busy || denied}
                          style={{
                            borderRadius: 10,
                            backgroundColor:
                              inGroup || denied
                          ...
                          {busy ? (
                            ...
                            <T
                              ...
                              style={{
                                fontSize: 10,
                                fontWeight: "800",
                                color: inGroup || denied ? d.faint : "#fff",
                              }}
                            >
                              {denied
                                ? "Can't add"
                  ...
                  {addResults && !addSearching && addResults.length === 0 ? (
                    ...
                    >
                      No accounts match “{addQuery.trim()}”.
                    </T>
                  ) : null}
                </>
              ) : (
                <>
                  <T
                    v="caption"
                    style={{ fontSize: 10, color: d.faint, marginBottom: 14 }}
                  >
                    From your connections and the DeenLink community
                  </T>
                  {ADDABLE.map((a) => {
        ...
        >
          ...
          >
            ...
            <T
              ...
              style={{ fontSize: 10, color: d.faint, marginBottom: 14 }}
            >
              Pick a style or upload your own photo — a default applies
              automatically
            </T>
            ...
            <Pressable
              ...
              style={{
                ...
                borderRadius: 14,
                borderWidth: 1.5,
                borderColor: "rgba(91,200,245,0.45)",
                backgroundColor: isDark
            ...
            {isGroupImg(group.cover) ? (
              ...
              >
                ...
                >
                  Remove uploaded photo (back to style)
                </T>
              </Pressable>
            ) : null}
            <View style={{ flexDirection: "row", gap: 8 }}>
              ...
              {COVER_STYLES.map((c) => {
                ...
                return (
                  <Pressable
                    key={c.id}
                    onPress={() => changeCover(c.id)}
                    style={{
                      flex: 1,
                      borderRadius: 12,
                      overflow: "hidden",
                      borderWidth: 2,
                      borderColor: on ? "#E8C96A" : "transparent",
                    }}
                  >
                    <LinearGradient
                      colors={c.grad}
                    ...
                    >
                      {on ? (
                        <FontAwesome5 name="check" size={13} color="#E8C96A" />
                      ) : null}
                    </LinearGradient>
                    <T
                      v="caption"
          ...
          >
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <T
                  v="h3"
                  style={{ fontWeight: "900", fontSize: 16, color: d.text }}
                >
                  Group settings
                </T>
                <T
                  v="caption"
                  style={{ fontSize: 10, color: d.faint, marginTop: 1 }}
                >
                  {isOwner
                    ? "You own this group"
                    : "Admin · some settings locked"}
                </T>
              </View>
              <Pressable
                onPress={onClose}
            ...
            <T v="caption" style={label}>
              BIO — SHOWN UNDER THE GROUP NAME
            </T>
            <TextInput
              value={bio}
              onChangeText={setBio}
              ...
              placeholderTextColor={d.faint}
              maxLength={90}
              style={{ ...input, fontFamily: "Poppins-Regular" }}
            />
              ...
              {CATS.map((c) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {c === "Organization" ? "ORG." : c.toUpperCase()}
                    </T>
                  </Pressable>
                );
              })}
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <T
                  v="bodyS"
                  style={{ fontWeight: "800", fontSize: 12, color: d.text }}
                >
                ...
                >
                  Square photo · replaces the emoji
                </T>
              </View>
              <FontAwesome5 name="chevron-right" size={10} color={d.faint} />
            </Pressable>
            {isGroupImg(avatar) ? (
              ...
              >
                ...
                <T
                  v="caption"
                  style={{ fontSize: 10, fontWeight: "700", color: "#E53E3E" }}
                >
                  Remove photo
                </T>
              </Pressable>
                ...
                return (
                  <Pressable
                    ...
                    style={{
                      ...
                      borderRadius: 14,
                      borderWidth: 2,
                      borderColor: on ? "#E8C96A" : d.cardBorder,
                      backgroundColor: on ? "rgba(212,175,55,0.1)" : d.bg,
                      alignItems: "center",
                      justifyContent: "center",
                    }}
                  >
                    <T v="h2" style={{ fontSize: 20 }}>
                      {a}
                    </T>
                  </Pressable>
                );
            ...
            >
              {isGroupImg(cover) ? (
                <ExpoImage
                  source={{ uri: cover }}
                  style={{ width: 40, height: 24, borderRadius: 7 }}
                  contentFit="cover"
                />
              ) : (
                <View
              ...
              {COVER_STYLES.map((c) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {on ? (
                        <FontAwesome5 name="check" size={12} color="#E8C96A" />
                      ) : null}
                    </LinearGradient>
                  </Pressable>
                );
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                >
                  {open ? "Anyone joins instantly" : "You approve each request"}
                </T>
              </View>
              <Switch
                value={open}
