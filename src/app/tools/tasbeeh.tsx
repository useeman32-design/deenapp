...
const PRESETS: Preset[] = [
  {
    ...
    arabic: "لَا إِلَٰهَ إِلَّا ٱللَّهِ",
    target: 99,
  },
  {
    id: "subhan",
...
export default function Tasbeeh() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const d = JSON.parse(
          (await storage.getItem("dl.tasbeeh.daily")) ?? "{}",
        );
        if (d.date === todayKey()) {
          setCounts(d.counts ?? {});
      ...
      try {
        const h = JSON.parse(
          (await storage.getItem("dl.tasbeeh.history")) ?? "{}",
        );
        setHistory((prev) => ({ ...h, ...prev }));
      } catch {}
      try {
        const p = JSON.parse(
          (await storage.getItem("dl.tasbeeh.prefs")) ?? "{}",
        );
        if (p.presetId && PRESETS.some((x) => x.id === p.presetId))
  ...
  const savePrefs = (patch: Record<string, unknown>) => {
    storage
      .setItem(
        "dl.tasbeeh.prefs",
        JSON.stringify({ presetId, target, vibrate, ...patch }),
      )
      .catch(() => {});
  };
  ...
  const bump = () => {
    ...
    storage
      .setItem(
        "dl.tasbeeh.daily",
        JSON.stringify({ date: todayKey(), counts: nc, total: nt }),
      )
      .catch(() => {});
    storage
      .setItem(
        "dl.tasbeeh.history",
        JSON.stringify({ ...history, [todayKey()]: nt }),
      )
      .catch(() => {});
    pulse.setValue(1.22);
  ...
  const week = useMemo(
    () =>
      Array.from({ length: 7 }, (_, i) => history[dayOffset(i)] ?? 0).reduce(
        (a, b) => a + b,
        0,
      ),
    [history],
  ...
  return (
    <View style={{ flex: 1, backgroundColor: BG }}>
      ...
      <LinearGradient
        colors={["rgba(20,74,45,0.32)", "rgba(5,13,9,0)"]}
        style={{ position: "absolute", top: 0, left: 0, right: 0, height: 320 }}
        pointerEvents="none"
      />
      <LinearGradient
        colors={["rgba(5,13,9,0)", "rgba(3,8,6,0.85)"]}
        style={{
          position: "absolute",
          bottom: 0,
          left: 0,
      ...
      >
        ...
        <View style={{ flex: 1, alignItems: "center" }}>
          ...
          <T
            v="caption"
            style={{ fontSize: 10.5, color: INK_FAINT, marginTop: 2 }}
          >
        ...
        <Pressable
          ...
          accessibilityLabel="tasbeeh settings"
          style={{
            width: 42,
            height: 42,
            borderRadius: 15,
      ...
      >
          ...
          <View style={{ alignItems: "center", marginTop: 8 }}>
            <View>
              ...
              <Animated.Text
                ...
                style={{
                  ...
                  fontSize: 19,
                  color: NEON,
                  opacity: floatUp.interpolate({
                    inputRange: [0, 0.25, 1],
                    outputRange: [0, 1, 0],
                  }),
                  transform: [
                    {
                      translateY: floatUp.interpolate({
                        inputRange: [0, 1],
                        outputRange: [10, -40],
                      }),
                    },
            ...
            >
              <T
                ...
                style={{
                  fontSize: 10.5,
                  color: "rgba(242,247,243,0.38)",
                  letterSpacing: 0.5,
                }}
              >
                {celebrate
                  ? "Tour complete — Alhamdulillah 🤍"
                  : `Tours: ${tours}`}
              </T>
              {tours > 0 ? (
                <Pressable
                  ...
                  onPress={() =>
                    setShareCard({
                      ...
                      metric: String(count),
                      title:
                        PRESETS.find((x) => x.id === presetId)?.label ??
                        "Dhikr",
                      subtitle: `${tours} tour${tours === 1 ? "" : "s"} complete · DeenLink Misbaha`,
                      link: "https://deenlink.org/tools/tasbeeh",
                    })
                  }
                  hitSlop={8}
                  style={{
        ...
        <Pressable
          accessibilityLabel="tasbeeh-deck"
        ...
        >
          <View style={{ borderRadius: 26, overflow: "hidden" }}>
            ...
            <Svg width={imgW} height={imgH} viewBox={`0 0 ${imgW} ${imgH}`}>
              ...
              <SvgG>
                <SvgCircle
                  cx={imgW / 2}
                  cy={imgH * 0.58 - Math.min(imgW, imgH) * 0.31 - 3}
                  r={5}
                  fill="url(#silk)"
                  stroke="#E8C96A"
                  strokeWidth={1}
                />
                <SvgPath
                  d={`M ${imgW / 2} ${imgH * 0.58 - Math.min(imgW, imgH) * 0.31 - 1} L ${imgW / 2} ${imgH * 0.58 - Math.min(imgW, imgH) * 0.31 + 7}`}
                  stroke="#E8C96A"
                  strokeWidth={3}
                  fill="none"
                />
                <SvgRect
                  x={imgW / 2 - 8}
                  y={imgH * 0.58 - Math.min(imgW, imgH) * 0.31 - 30}
                  width={16}
                  height={18}
                  rx={5}
                  fill="url(#silk)"
            ...
            <LinearGradient
              colors={["rgba(5,13,9,0.55)", "rgba(5,13,9,0)"]}
              style={{
                position: "absolute",
                top: 0,
                left: 0,
            ...
            />
            <LinearGradient
              colors={["rgba(5,13,9,0)", "rgba(5,13,9,0.6)"]}
              style={{
                position: "absolute",
                bottom: 0,
                left: 0,
          ...
          >
            ...
            <T
              v="caption"
              style={{ fontSize: 11, color: INK_FAINT, letterSpacing: 0.3 }}
            >
              Swipe or tap on the beads to count
            </T>
          </View>
        </Pressable>
        ...
        {/* ── target number ── */}
        <View
          style={{ ...glass, marginHorizontal: 16, marginTop: 12, padding: 15 }}
        >
          <T
            v="caption"
            style={{
          ...
          <View style={{ flexDirection: "row", gap: 8 }}>
              ...
              return (
                <Pressable
                  ...
                  style={{
                    ...
                    borderWidth: 1,
                    borderColor: on
                      ? "rgba(74,227,143,0.65)"
                      : "rgba(255,255,255,0.07)",
                    backgroundColor: on
                      ? "rgba(74,227,143,0.10)"
            ...
            <Pressable
              ...
              style={{
                ...
                borderRadius: 14,
                borderWidth: 1,
                borderColor: [33, 66, 99, 100].includes(target)
                  ? "rgba(255,255,255,0.07)"
                  : "rgba(74,227,143,0.65)",
                backgroundColor: [33, 66, 99, 100].includes(target)
                  ? "rgba(255,255,255,0.03)"
                  : "rgba(74,227,143,0.10)",
              }}
            >
              <T
                ...
                style={{
                  fontSize: 14,
                  fontWeight: "800",
                  color: [33, 66, 99, 100].includes(target) ? INK_FAINT : NEON,
                }}
              >
      ...
      >
        <Pressable
          style={{
            flex: 1,
            backgroundColor: "rgba(2,6,4,0.75)",
            alignItems: "center",
            justifyContent: "center",
            padding: 22,
          }}
      ...
      >
        ...
        >
          <Pressable
            ...
            onPress={(e) => stopBubble(e)}
          >
            <T v="h3" style={{ fontSize: 15, fontWeight: "800", color: INK }}>
              Custom target
            </T>
            <T
              v="caption"
            ...
            >
              Any number between 3 and 9,999.
            </T>
            <TextInput
              value={customTxt}
