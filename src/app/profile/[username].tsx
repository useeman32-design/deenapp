...
import {
  ...
  isLive,
  reportAccount,
  toggleFollow as srvToggleFollow,
  userPosts as srvUserPosts,
  type PublicProfile,
} from "@/api/client";
...
function PublicProfileScreenInner() {
  ...
  const [tab, setTab] = useState<ProfileTab>(
    initialTab === "videos" || initialTab === "questions"
      ...
      .then((rows) => {
        ...
        setUserReels(
          (rows ?? [])
            .filter(
              (v) =>
                String((v as { username?: string }).username ?? "") ===
                username,
            )
            .map((v, i) => ({
              ...
              src: {
                uri: String(
                  (v as { sourceUrl?: string; source_url?: string })
                    .sourceUrl ??
                    (v as { source_url?: string }).source_url ??
                    "",
                ),
              },
              poster: {
                uri: String(
                  (v as { posterUrl?: string; poster_url?: string })
                    .posterUrl ??
              ...
              username,
              accountName: String(
                (v as { accountName?: string }).accountName ?? username,
              ),
              accountPic: ((v as { accountPic?: string | null }).accountPic ??
                null) as string | null,
              caption: String(
  ...
  const profile: MockProfile | null = useMemo(() => {
    ...
    if (liveP) {
      return {
        ...
        bio:
          liveP.bio ?? "" /* pass 83-3: no fake bio — empty renders "No bio" */,
  ...
  const isOwnProfile =
    liveP?.id != null &&
    user?.id != null &&
    Number(liveP.id) === Number(user.id);

  const posts = useMemo(() => livePosts ?? [], [username, livePosts]);
  ...
  if (!profile) {
    if (!ready || liveLoading) {
      ...
      return (
        <BreathingContent
          bar={isDark ? "rgba(255,255,255,0.08)" : "rgba(20,36,28,0.08)"}
          bg={d.bg}
        />
      );
    }
    return (
      ...
      >
        ...
        <T
          v="bodyS"
          style={{ color: d.subtext, fontSize: 13, fontWeight: "600" }}
        >
  ...
  const TABS: Array<{ id: ProfileTab; label: string }> = [
    ...
    ...(isScholar
      ? [{ id: "questions" as ProfileTab, label: "Questions" }]
      : []),
    ...
    <View style={{ flex: 1, backgroundColor: d.bg }}>
          ...
          {menuOpen ? (
            <>
              ...
              >
                  ...
                  onPress={() => {
                    ...
                    void blockUser(liveP.username, !iBlocked).then((ok) => {
                      ...
                      if (ok) {
                        setIBlocked((v) => !v);
                        Alert.alert(
                          iBlocked ? "Unblocked" : `Blocked @${liveP.username}`,
                          iBlocked
                            ? "They can message and find you again."
                            : "They can no longer message, follow or find you. Manage this in Settings → Privacy & Safety.",
                        );
                      } else {
                        Alert.alert(
                          "Could not update",
                ...
                >
                  <FontAwesome5
                    name={iBlocked ? "user-check" : "user-slash"}
                    size={12}
                    color="#E05252"
                  />
                  <T
                    v="bodyS"
                    style={{ fontSize: 12.5, fontWeight: "700", color: d.text }}
                  >
                    {blockBusy
                      ? "Working…"
                      : iBlocked
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  <AvatarImage
                    ...
                    border="transparent"
                    gender={
                      ((liveP as { gender?: string } | null)?.gender ??
                        null) as string | null
                    }
                  />
                </Pressable>
              ...
              <View style={{ flex: 1, minWidth: 0, gap: 4 }}>
                ...
                >
                  ...
                  {profile.badge ? (
                    <VerificationBadge type={profile.badge} size={14} />
                  ) : null}
                </View>
                <T
                  v="caption"
              ...
              {[
                {
                  label: "Posts",
                  value: fmt(
                    Math.max(posts.length, liveP?.posts ?? profile.posts_count),
                  ),
                  tab: null,
                },
                {
                  ...
                  value: fmt(
                    followerCount +
                      (following !== !!liveP?.following_by_me
                        ? following
                          ? 1
                          : -1
                        : 0),
            ...
            <View style={{ flexDirection: "row", gap: 9 }}>
              ...
              >
                ...
                <T
                  v="body"
                  style={{ color: d.subtext, fontWeight: "800", fontSize: 12 }}
                >
                  Share
                </T>
              </Pressable>
              ...
              posts.map((p) => (
                <FeedCard
                  ...
                  post={{
                    ...
                    like_count:
                      (p.like_count ?? 0) + (likedPosts.has(p.id) ? 1 : 0),
                  }}
                  ...
                  onDelete={
                    isOwnProfile
                      ? () => {
                          void srvDeletePost(p.id).then((ok) => {
                            if (ok)
                              setLivePosts((prev) =>
          ...
          <View style={{ marginHorizontal: 16, gap: 12 }}>
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <T
                  v="bodyS"
                  style={{ color: d.text, fontWeight: "700", fontSize: 12.5 }}
                >
                  Ask this scholar a question
                </T>
                <T
                  v="caption"
                  style={{ color: d.subtext, fontSize: 11, marginTop: 2 }}
                >
          ...
          <View style={{ marginHorizontal: 16, gap: 12 }}>
            {videoPosts.length === 0 ? (
              ...
              >
                ...
                >
                  No videos yet — video posts from this account will appear
                  here.
                </T>
              </View>
            ) : (
                    ...
                    >
                      <View
                        style={{
                          ...
                          height: 34,
                          borderRadius: 17,
                          backgroundColor: "rgba(4,12,8,0.55)",
                          borderWidth: 1,
      ...
      >
        <Pressable
          style={{
            flex: 1,
            backgroundColor: "rgba(4,8,6,0.95)",
            alignItems: "center",
            justifyContent: "center",
          }}
      ...
      <ContentShareSheet
        ...
        card={{
          kind: "profile",
          meaning: name,
          ref: `@${profile.username}${profile.bio ? ` · ${profile.bio}` : ""}`,
      ...
      >
        ...
        >
          ...
          >
            ...
            <T
              v="caption"
              style={{ fontSize: 11, color: d.faint, marginBottom: 12 }}
            >
  ...
  return (
    ...
    >
      ...
      <View style={{ gap: 12, marginTop: 10 }}>
        {Array.from({
          length: Math.max(2, Math.min(4, Math.floor((height - 420) / 92))),
        }).map((_, i) => (
          <View
            key={i}
            style={{ height: 80, borderRadius: 16, backgroundColor: bar }}
          />
        ))}
      </View>
    </Animated.View>
