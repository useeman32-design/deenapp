 ...
 * nested replies w/ thread line) → "Replying to @user" indicator →
...
export function CommentsModal({
  ...
  const addComment = () => {
    ...
    setItems((prev) => {
      if (replyingTo) {
        return prev.map((c) => {
          // reply to a top-level comment → its replies list
          if (c.id === replyingTo.id) {
            return { ...c, replies: [...(c.replies ?? []), nc] };
          }
          // reply to a nested reply → extend the parent's replies
  ...
  const CommentRow = ({ c, isReply = false }: { c: SampleComment; isReply?: boolean }) => {
    ...
    const likeCount = (c.likes ?? 0) + (isLiked && !c.liked ? 1 : 0);
      ...
      <View style={{ flexDirection: 'row', gap: 9, marginTop: isReply ? 8 : 14 }}>
        ...
        <View style={{ flex: 1, minWidth: 0 }}>
          <View style={{ borderRadius: 13, backgroundColor: bubble, paddingHorizontal: 11, paddingVertical: 7 }}>
            ...
            <MentionText
              text={c.text}
              base={{ fontSize: 12.5, lineHeight: 17.5, color: txt, marginTop: 2 }}
              mention={{ fontSize: 12.5, fontWeight: '800', color: emerald }}
            />
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginTop: 5 }}>
            ...
            <Pressable hitSlop={6} onPress={() => { haptic.light(); toggleLike(c.id); }} style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
              <HeartIcon size={11} filled={isLiked} color={isLiked ? '#E74C3C' : faint} />
              {likeCount > 0 ? (
                <T v="caption" style={{ fontSize: 9.5, color: isLiked ? '#E74C3C' : faint, fontWeight: '700' }}>
                  {likeCount}
                </T>
              ) : null}
            </Pressable>
            <Pressable hitSlop={6} onPress={() => startReply(c)}>
