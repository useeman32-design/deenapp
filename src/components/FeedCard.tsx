      ...
      (entries) =>
        setInView(
          !!entries[0]?.isIntersecting &&
            (entries[0]?.intersectionRatio ?? 0) >= 0.28,
        ),
  ...
  return (
    <View ref={host} style={{ height, width: "100%" }}>
      {active ? (
        ...
        >
          <FontAwesome5 name="pause" size={18} color="rgba(255,255,255,0.55)" />
        </View>
      )}
    </View>
  );
...
export function AvatarImage({
  ...
  const isDefaultName =
    str0 === "" ||
    str0 === "default_profile.jpg" ||
    str0.endsWith("/img/default_profile.jpg");
  const uri =
...
function VideoPostPlayer({
  ...
  useEffect(() => {
    const sub = (
      player.addListener as (
        ev: string,
        cb: (st: { status?: string }) => void,
      ) => { remove: () => void }
    )("statusChange", (st) => {
      if (st?.status === "playToEnd") {
  ...
  const seekPan = () =>
    PanResponder.create({
      ...
      onPanResponderGrant: (e) => {
        ...
        const f = Math.max(
          0,
          Math.min(1, e.nativeEvent.locationX / (barW.current || 300)),
        );
  ...
  useEffect(() => {
    const t = player.addListener(
      "timeUpdate",
      (st: { currentTime: number; duration?: number }) => {
  ...
  const renderBar = (pill: boolean) => (
    ...
    >
      ...
      >
        {mmss((dragging ? dragFrac : frac) * dur)}
      </T>
      <View
        {...seekPan().panHandlers}
      ...
      >
        ...
        <View
          style={{
            position: "absolute",
            left: 0,
            width: `${(dragging ? dragFrac : frac) * 100}%`,
            height: 4,
            borderRadius: 2,
        ...
        <View
          style={{
            position: "absolute",
            left: `${(dragging ? dragFrac : frac) * 100}%`,
            marginLeft: -5.5,
            width: 11,
            height: 11,
            borderRadius: 6,
          ...
          >
            <View
              style={{
                ...
                height: 58,
                borderRadius: 29,
                backgroundColor: "rgba(4,12,8,0.6)",
                borderWidth: 1.5,
...
export function FeedCard({
  ...
  const fieldLabel =
    field ||
    (user as { fields?: string | null }).fields ||
    user.scholar?.fields_of_knowledge ||
    null;
  ...
  const media =
    (post.media ?? []).find(
      (m) =>
        String(
          (m as Record<string, unknown>).media_type ?? m.type ?? "image",
        ) === "image",
  ...
  const mediaImgs = (post.media ?? [])
    .map((m) => {
      const mm = m as { url?: unknown; video_url?: unknown };
      return mm.video_url == null && mm.url != null ? String(mm.url) : null;
    })
    .filter((u): u is string => u != null);

  const fullText = post.content_text ?? "";
  const longText = fullText.length > 230;
  const cutAt = longText
    ? Math.max(fullText.lastIndexOf(" ", 230), 180)
    : fullText.length;
  const shownText =
    expanded || !longText ? fullText : `${fullText.slice(0, cutAt).trimEnd()}…`;

  const scale = burst.interpolate({
    inputRange: [0, 0.6, 1],
    outputRange: [0.22, 1.02, 1.3],
  });
  ...
  const submitReport = () => {
    ...
    Alert.alert(
      "Report submitted",
      "JazakAllah khair — our moderation team will review this post.",
    );
  };
    ...
    >
        ...
        >
          ...
          >
            {group.avatar && /^data:|^file:|^https?:/.test(group.avatar) ? (
              ...
              <FontAwesome5
                name={(group.catIcon ?? "users") as never}
                size={12}
                color="#E8C96A"
              />
            )}
      ...
      >
        <Pressable
          ...
          onPress={() => {
            ...
            router.push({
              ...
              params: { username: String(user.username) },
            } as never);
          }}
        ...
        >
          <AvatarImage
            ...
            size={42}
            tint={`${accent}26`}
            border={dash ? dash.greenBorder : hairline}
            gender={(user as { gender?: string }).gender ?? null}
          />
        </Pressable>
            ...
            {post.group_name ? (
              ...
              >
                ...
                <T
                  v="caption"
                  numberOfLines={1}
                  style={{ fontSize: 9.5, color: accent, fontWeight: "700" }}
                >
                  {post.group_name}
                </T>
              </View>
        ...
        >
          <T
            v="caption"
            style={{ color: faint, fontSize: 16, fontWeight: "700" }}
          >
            •••
          </T>
        </Pressable>
        ...
        >
          ...
          >
            ...
            >
              ...
              <T
                v="bodyS"
                style={{ fontSize: 12, fontWeight: "600", color: txt }}
              >
                Report
              </T>
            </Pressable>
            <Pressable
              onPress={() => {
                ...
                Alert.alert(
                  "Not interested",
                  `You’ll see fewer posts like ${name}’s.`,
                  [
      ...
      {fullText ? (
        <View style={{ marginBottom: 12 }}>
          <Pressable onPress={() => onTap()}>
            <T
              v="bodyS"
              style={{ fontSize: 13.5, lineHeight: 20.5, color: txt }}
            >
              {shownText}
            </T>
          </Pressable>
          ...
          >
            ...
            >
              ...
              >
                <FontAwesome5
                  name="poll-h"
                  size={10}
                  color={dash ? dash.emerald : accent}
                />
              </View>
              {post.poll.question ? (
                <T
              ...
              >
                {post.poll.duration
                  ? `${pollDurationLabel(post.poll.duration)} left`
                  : "2d left"}
              </T>
            </View>
            ...
            {pollState.options.map((opt) => {
              ...
              const topVotes = Math.max(
                ...pollState.options.map((o) => o.votes),
              );
              const leads =
                total > 0 && opt.votes === topVotes && opt.votes > 0;
              const fill = mine
                ? dash
                  ? `${gold}30`
                  : `${accent}26`
                  ...
                  onPress={() => {
                    ...
                    setPollState((prev) => {
                      ...
                      return {
                        ...
                        options: prev.options.map((o) => {
                          let votes = o.votes;
                          if (prev.voted != null && o.id === prev.voted)
                ...
                >
                  ...
                  {voted ? (
                    <T
                      v="caption"
                      style={{
                        color: mine ? (dash ? gold : accent) : sub,
                        fontSize: 11,
                        fontWeight: "800",
                        flexShrink: 0,
                      }}
      ...
      {post.image_url && mediaImgs.length <= 1 ? (
        <Pressable
          onPress={() =>
            onTap(() => {
              setPreviewIdx(mediaImgs.length > 1 ? carouselPage : 0);
              setImgPreview(true);
            })
          }
        ...
        >
          <ScrollView
            ...
            onMomentumScrollEnd={(e) =>
              setCarouselPage(
                Math.round(
                  e.nativeEvent.contentOffset.x / Math.max(1, carouselW),
                ),
              )
            }
            ...
            onScroll={(e) => {
              ...
              if (i !== carouselPage && i >= 0 && i < mediaImgs.length) {
                setCarouselPage(i);
              }
            }}
          >
            ...
            {mediaImgs.map((_, i) => (
              <View
                ...
                style={{
                  ...
                  borderRadius: 3,
                  backgroundColor:
                    i === carouselPage ? (dash ? gold : accent) : hairline,
                }}
              />
            ))}
          </View>
      ...
      {post.audio_url ? (
        <View style={{ marginBottom: 12 }}>
          <AudioCassette url={String(post.audio_url)} />
        </View>
      ) : null}
      ...
      {Platform.OS === "web" && post.youtube_embed_url ? (
        ...
        >
          <YouTubePlayer
            embedUrl={String(post.youtube_embed_url)}
            height={206}
          />
        </View>
      ) : post.youtube_url ? (
        <Pressable
          onPress={() => {
            ...
            if (post.youtube_url)
              Linking.openURL(post.youtube_url).catch(() => {});
          }}
          style={({ pressed }) => ({
            flexDirection: "row",
            alignItems: "center",
      ...
      {showActions ? (
        ...
        >
          <Pressable
            onPress={() => {
              if (guestBlock("Sign in to like and react to posts.")) return;
              haptic.light();
              onLike?.(post.id);
            }}
            hitSlop={8}
          ...
          <Pressable
            onPress={() => {
              if (guestBlock("Sign in to comment on posts.")) return;
              onComments?.(post);
            }}
          ...
          >
            ...
            <T
              v="caption"
              style={{ fontWeight: "600", fontSize: 14, color: sub }}
            >
          ...
          >
            <BookmarkIcon
              size={20}
              filled={savedNow}
              color={savedNow ? (dash ? "#E8C96A" : "#B8860B") : sub}
            />
          </Pressable>
        </View>
      ) : null}
      ...
      <ContentShareSheet
        ...
        card={{
          kind: "post",
          arabic: "",
          meaning: post.content_text ?? `${name} on DeenLink`,
          ref: `@${user.username} · DeenLink`,
          route: post.id > 0 ? `/tools/post?id=${post.id}` : undefined,
          authorName: name,
                ...
                setPreviewIdx(
                  Math.round(
                    e.nativeEvent.contentOffset.x /
                      Math.max(1, Dimensions.get("window").width),
                  ),
                )
              }
              renderItem={({ item }) => (
                <Image
                  source={
                    typeof item === "number" ? item : { uri: String(item) }
                  }
                  style={{ width: Dimensions.get("window").width, height: 560 }}
                  resizeMode="contain"
                />
              )}
              showsHorizontalScrollIndicator={false}
          ...
          >
            ...
            <View style={{ flexDirection: "row", gap: 10, marginTop: 14 }}>
              ...
              >
                <T
                  v="bodyS"
                  style={{ fontWeight: "600", fontSize: 12.5, color: sub }}
                >
                  Cancel
                </T>
              </Pressable>
              ...
              >
                <T
                  v="bodyS"
                  style={{ fontWeight: "700", fontSize: 12.5, color: "#fff" }}
                >
                  Submit report
                </T>
              </Pressable>
