...
function SwipeReply({
  ...
  const tx = x.interpolate({
    inputRange: [0, 60, 84, 200],
    outputRange: [0, 60, 72, 84],
    extrapolate: "clamp",
  });
  ...
  const nodeRef = useRef<{
    addEventListener?: (t: string, f: (e: { clientX: number }) => void) => void;
    removeEventListener?: (
      t: string,
      f: (e: { clientX: number }) => void,
    ) => void;
  } | null>(null);
...
function MenuRow({
  ...
  return (
    ...
    >
      ...
      <T v="bodyS" style={{ fontSize: 13.5, fontWeight: "600", color }}>
        {label}
      </T>
    </Pressable>
  );
...
function FadeIn({
  ...
  return (
    <Animated.View style={[style, { opacity: a }]}>{children}</Animated.View>
  );
}
...
export function CommunityInbox({
  ...
  const lastCidError =
    useRef(""); /* pass 83-12 — why the last conversation-open failed */
  /* pass 81 — never show bundled demo threads once we know we're live */
  useEffect(() => {
    if (live)
      setThreads((prev) => prev.filter((t) => !SEED_NAMES.has(t.friend)));
  }, [live]);
  ...
  const [reqMap, setReqMap] = useState<
    Record<string, { convId: number; photo?: string | null; name?: string }>
  >({});
  const [peerMap, setPeerMap] = useState<
    Record<string, { id?: number; name?: string; photo?: string | null }>
  >({});
  ...
  const [blockFlags, setBlockFlags] = useState<
    Record<string, { b: boolean; by: boolean }>
  >({});
  ...
  useEffect(() => {
    ...
    if (prev && prev !== openFriend) {
      const cid = convIds[prev];
      if (cid) {
        markRead(cid);
      }
    }
  ...
  useEffect(() => {
    if (initialFriend) setOpenFriend(initialFriend);
  }, [initialFriend]);
  ...
  useEffect(() => {
    setBlocked(openFriend ? !!blockFlags[openFriend]?.b : false);
  }, [openFriend, blockFlags]);
  /* pass 83-5 — mutual-follow suggestions under the empty state */
  const [sugg, setSugg] = useState<
    { username: string; name: string; photo?: string | null }[]
  >([]);
  ...
  const [unreadMap, setUnreadMap] = useState<Record<string, number>>(
    {},
  ); /* pass 83-14 — true unread counts from the server */
    ...
    storage.getItem(STORE).then((r) => {
      if (r)
        try {
          ...
          setThreads(
            parsed.filter((t) => !SEED_NAMES.has(t.friend)),
          ); /* scrub legacy demo threads */
        } catch {}
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const subs = [
      Keyboard.addListener("keyboardDidShow", on),
      Keyboard.addListener("keyboardDidHide", off),
    ];
  ...
  const acc = (u: string) => ({
    username: u,
    full_name: peerMap[u]?.name || u,
    photo: peerMap[u]?.photo ?? null,
  }); /* pass 83-38 — real peers only */
  ...
  const dispName = (u: string) =>
    blockFlags[u]?.by ? "DeenLink User" : acc(u).full_name;
        ...
        .then((rows) =>
          setSearchRes(
            (rows ?? [])
              ...
              .map((r) => ({
                username: r.username,
                name: (r as { full_name?: string }).full_name || r.username,
                photo:
                  (r as { profile_image_url?: string | null })
  ...
  const resolveCid = async (who: string): Promise<number | null> => {
    ...
    if (made && made.cid > 0) {
      ...
      if (made.status === "request") {
        setOutRequests((prev) => new Set(prev).add(who));
      }
  ...
  const react = (id: string, e: string) => {
    ...
    persist(
      threads.map((t) =>
        t.friend === thread.friend ? { ...t, reactions } : t,
      ),
    );
    if (focus) {
      closeFocus();
    ...
    void (async () => {
      ...
      if (!cid) {
        revertReaction(who, id, prev);
        return;
      }
      const r = await chatReact(cid, target.kind, target.id, next).catch(
        () => ({ ok: false, emoji: null }),
      );
      if (!r.ok) {
        revertReaction(who, id, prev);
        ...
        t.friend === who
          ? kind === "msg"
            ? {
                ...t,
                chat: t.chat.map((c) =>
                  c.id === id ? { ...c, deleted: true, text: "" } : c,
                ),
              }
            : {
                ...t,
                items: t.items.map((x) =>
                  x.id === id ? { ...x, deleted: true, title: "" } : x,
                ),
              }
          : t,
      ),
  ...
  const forwardTo = (
    to: string,
    payload: { kind: "msg" | "share"; text: string; kindOf?: Kind },
  ) => {
    haptic.success();
    const tmp = uid();
    setThreads((prev) =>
            ...
            : {
                ...
                items: [
                  ...t.items,
                  {
                    id: tmp,
                    rk: tmp,
                    kind: (payload.kindOf ?? "post") as Kind,
                    title: payload.text,
                    ago: ago(),
                    dir: "me" as const,
                    at: "",
    ...
    void (async () => {
      ...
      const made =
        payload.kind === "msg"
          ...
          : await chatSendShare(
              cid,
              String(payload.kindOf ?? "post"),
              payload.text,
            ).catch(() => null);
      if (!made || !("id" in made) || !made.id) {
        return;
      }
  ...
  useEffect(() => {
    ...
    getConnections("following")
      .then((r) => {
        ...
        setSugg(
          r.items
            .filter((i) => i.follows_me && !i.is_me)
            .slice(0, 6)
            .map((i) => ({
              username: i.username,
              name: i.name || i.username,
              photo: i.profile_image_url ?? null,
            })),
        );
      })
      .catch(() => {});
  ...
  const refreshConvs = () => {
    ...
    return chatConversations()
      .then((cs) => {
        ...
        const reqs: Record<
          string,
          { convId: number; photo?: string | null; name?: string }
        > = {};
        const peers: Record<
          string,
        ...
        cs.forEach((c) => {
          ...
          if (c.peer_seen) {
            m[u] = String(c.peer_seen);
          }
          peers[u] = {
            id: c.peer?.id,
            name: c.with_name || undefined,
          ...
          if (st === "request") {
            if (
              c.requested_by != null &&
              user?.id != null &&
              c.requested_by !== user.id
            ) {
              reqs[u] = {
                convId: c.id,
                photo: c.with_photo ?? null,
                name: c.with_name || c.title || u,
              };
              return; /* not a main-list thread until accepted */
            }
            mine.add(
              u,
            ); /* my outgoing request stays visible in the main list */
          }
          ids[u] = c.id;
        });
        ...
        cs.forEach((c) => {
          ...
          if (ids[u2] && (readOkAt.current[ids[u2]] ?? 0) >= startedAt) {
            un[u2] = 0;
          }
        });
        setUnreadMap(un);
        ...
        if (live) {
          setThreads((prev) => {
            ...
            const add: Thread[] = Object.keys(ids)
              .filter((u) => !have.has(u))
              .map((u) => ({
                friend: u,
                items: [],
  ...
  useEffect(() => {
    ...
    if (threads.some((t) => t.friend === openFriend)) {
      return;
    }
    let dead = false;
    const who = openFriend;
    setThreads((prev) =>
      prev.some((t) => t.friend === who)
        ? prev
  ...
  const actOnRequest = async (
    ...
    const ok =
      action === "block"
        ? (await blockUser(uname, true).catch(() => false)) &&
          (await chatRequestAction(r.convId, "block").catch(() => false))
        : await chatRequestAction(r.convId, action).catch(() => false);
    setReqBusy(null);
    if (!ok) {
    ...
    if (action === "accept") {
      ...
      if (action === "report") {
        Alert.alert(
          "Reported",
          "JazakAllah khair — our moderation team will review this account.",
        );
      }
      await refreshConvs();
      if (Object.keys(reqMap).length <= 1) {
        setRequestsOpen(false);
      }
    }
  };
  ...
  useEffect(() => {
    ...
    chatThread(cid)
      ...
      .then((data) => {
        ...
        lastMsgId.current = data.messages.reduce(
          (mx, m) => Math.max(mx, m.id),
          0,
        );
        lastShareId.current = data.shares.reduce(
          (mx, x) => Math.max(mx, x.id),
          0,
        );
        ...
        const chat: ChatMsg[] = data.messages.map((m) => ({
          id: `s${m.id}`,
          text: m.body,
          ago: (m.created_at || "").slice(11, 16),
          dir: m.sender_id === user?.id ? ("me" as const) : ("them" as const),
          at: m.created_at || "",
          deleted: !!m.deleted,
          reply: m.reply_to
            ? { who: m.reply_to.username ?? "", text: m.reply_to.body }
            : null,
        ...
        const items: ShareItem[] = data.shares.map((s) => ({
          id: `h${s.id}`,
          kind: (Object.keys(KIND_META) as Kind[]).includes(s.kind as Kind)
            ? (s.kind as Kind)
            : ("post" as Kind),
          title: s.title,
          ...
          sub: s.payload?.sub,
          route:
            typeof s.payload?.route === "string" ? s.payload.route : undefined,
          dur: s.payload?.dur,
        }));
    ...
    const tick = () => {
      ...
      chatThread(cid, { msg: lastMsgId.current, share: lastShareId.current })
        .then((data) => {
          ...
          if (newShares.length) {
            lastShareId.current = Math.max(
              lastShareId.current,
              ...newShares.map((x) => x.id),
            );
          }
          const wm = data.peer_read_at ?? null;
          ...
          const addShares: ShareItem[] = newShares.map((x) => ({
            id: `h${x.id}`,
            kind: (Object.keys(KIND_META) as Kind[]).includes(x.kind as Kind)
              ? (x.kind as Kind)
              : ("post" as Kind),
            title: x.title,
            ago: (x.created_at || "").slice(11, 16),
            dir: x.sender_id === user?.id ? ("me" as const) : ("them" as const),
            at: x.created_at || "",
            deleted: !!x.deleted,
            arabic: x.payload?.arabic,
            refLabel: x.payload?.refLabel,
            sub: x.payload?.sub,
            dur: x.payload?.dur,
          }));
          const incoming =
            addChat.some((c) => c.dir === "them") ||
            addShares.some((c) => c.dir === "them");
          if (incoming) {
          ...
          if (newMsgs.length || newShares.length || wm) {
            setThreads((prev) =>
              prev.map((t) => {
                ...
                let chat = [
                  ...t.chat,
                  ...addChat.filter((c) => !have.has(c.id)),
                ];
                const items = [
                  ...t.items,
                  ...addShares.filter((c) => !haveS.has(c.id)),
                ];
                if (wm) {
                  chat = chat.map((c) =>
                    c.dir === "me" &&
            ...
            if (incoming && atBottomRef.current) {
              smoothRef.current?.();
            }
            if (incoming && Date.now() - lastAutoRead.current > 2000) {
              /* pass 83-23 — 10s left fast messages unread server-side */
              lastAutoRead.current = Date.now();
              markRead(cid);
            }
          }
          setPeerTyping((prevDots) =>
            prevDots === !!data.peer_typing ? prevDots : !!data.peer_typing,
          );
        })
        .catch(() => {})
        .finally(() => {
  ...
  const isOnline = useCallback(
    (u: string | null | undefined) => {
      ...
      return (
        !!t &&
        Date.now() - new Date(t.replace(" ", "T")).getTime() < 5 * 60 * 1000
      );
    },
    [seenMap, blockFlags],
  );
    ...
    const els = [...rootEl.querySelectorAll("*")].filter(
      (e) =>
        (e as HTMLElement).scrollHeight >
          (e as HTMLElement).clientHeight + 40 &&
  ...
  const smoothScrollBottom = () => {
    ...
    [0, 60, 160, 300].forEach((t) =>
      setTimeout(() => scroller.current?.scrollToEnd({ animated: true }), t),
    );
  ...
  const sendChat = () => {
    ...
    const chat = [
      ...thread.chat,
      {
        ...
        dir: "me" as const,
        at: "",
        reply: quote ? { who: quote.who, text: quote.text } : null,
      },
    ];
    persist(
      threads.map((t) => (t.friend === thread.friend ? { ...t, chat } : t)),
    );
    ...
    if (live) {
      ...
      void (async () => {
        const cid = await resolveCid(who);
        if (!cid) {
          markFailed(who, id, lastCidError.current);
          return;
        }
        ...
        if (!sent || !sent.id) {
          ...
          const short =
            sent?.errorCode === "request_limit" || outRequests.has(who)
              ? `request pending · max 3 messages until @${who} accepts`
              : sent?.errorCode === "declined"
                ? "conversation closed"
                : sent?.errorCode === "blocked"
                  ? "blocked"
                  : why
                    ? why.slice(0, 60)
                    : "";
          ...
          if (sent?.errorCode === "request_limit" || outRequests.has(who)) {
            Alert.alert(
              "Request pending",
              why ??
                `You can send up to 3 messages until @${who} accepts your request.`,
            );
          } else if (sent?.errorCode === "declined") {
            Alert.alert(
              "Conversation closed",
              why ?? "This conversation is closed.",
            );
          } else if (sent?.errorCode === "blocked") {
            setBlocked(
              true,
            ); /* pass 83-14 — the menu now offers Unblock right here */
            Alert.alert(
              "Cannot send",
              why ?? "You can no longer message this account.",
            );
          } else if (why) {
          ...
          ? {
              ...
              chat: t.chat.map((c) =>
                c.id === id
                  ? {
                      ...c,
                      text: `${c.text}  ⚠ not sent${reason ? " — " + reason : ""}`,
                    }
                  : c,
              ),
            }
  ...
  const shareBack = (
    ...
    const items = [
      ...thread.items,
      {
        ...
        at: "",
        route:
          typeof (payload as { route?: unknown } | undefined)?.route ===
          "string"
            ? ((payload as { route?: string }).route as string)
            : undefined,
      },
    ];
    persist(
      threads.map((t) => (t.friend === thread.friend ? { ...t, items } : t)),
    );
    smoothScrollBottom();
    ...
    if (live) {
      ...
      void (async () => {
        const cid = await resolveCid(who);
        if (!cid) {
          markShareFailed(who, id, lastCidError.current);
          return;
        }
        const made = await chatSendShare(cid, kind, title, payload).catch(
          () => null,
        );
        if (!made) {
          markShareFailed(who, id);
          return;
        }
        lastShareId.current = Math.max(
          lastShareId.current,
          ...
          ? {
              ...
              items: t.items.map((x) =>
                x.id === id
                  ? {
                      ...x,
                      title: `${x.title}  ⚠ not sent${reason ? " — " + reason : ""}`,
                    }
                  : x,
              ),
            }
  ...
  const renderShare = (th: Thread, it: ShareItem) => {
    ...
    return (
      <Animated.View
        key={it.rk ?? it.id}
        style={{ opacity: isFocus ? 1 : dim }}
      >
        <SlideIn
          animate={freshIds.current.has(it.id)}
          style={{
            flexDirection: "row",
        ...
        >
          {!mine ? (
            <AvatarImage
              source={acc(th.friend).photo ?? null}
              name={acc(th.friend).full_name}
              size={28}
              tint="rgba(46,204,113,0.2)"
              border={d.cardBorder}
            />
          ) : null}
          ...
          <SwipeReply
            onReply={() => openReply(it.id)}
            tint={isDark ? "#4AE38F" : "#1D6F42"}
            style={{ maxWidth: "76%" }}
          >
            <Pressable
              ref={(r) => {
                rowRefs.current[it.id] = r as never;
              }}
              onPress={() => {
                ...
                if (it.route) {
                  if (!standalone) {
                    storage
                      .setItem("dl_inbox_reopen", th.friend)
                      .catch(() => {});
                    onNavigateAway?.();
                  }
              ...
              style={({ pressed }) => ({
                ...
                ...(isFocus
                  ? {
                      shadowColor: "rgba(74,227,143,0.5)",
                      shadowOpacity: 0.9,
                      shadowRadius: 22,
            ...
            >
              ...
              {it.deleted ? (
                ...
                >
                  ...
                  <View style={{ padding: 8 }}>
                    <T
                      v="bodyS"
                      style={{ fontSize: 12, lineHeight: 17, color: d.text }}
                    >
                      {it.title}
                    </T>
                    {it.sub ? (
                      <T
                        v="caption"
                        style={{ fontSize: 9, color: d.faint, marginTop: 4 }}
                      >
                        {it.sub}
                      </T>
                    ) : null}
                  ...
                  >
                    ...
                    <T
                      ...
                      style={{
                        fontSize: 8.5,
                        fontWeight: "800",
                        color: it.kind === "ayah" ? "#B8870B" : d.faint,
                        letterSpacing: 0.4,
                      }}
                    >
                      {it.refLabel ?? it.title}
                    </T>
                    <View
                      style={{
                        width: 14,
                ...
                <View
                  style={{
                    ...
                    borderRadius: 12,
                    borderWidth: 1,
                    borderColor: "rgba(143,211,182,0.5)",
                    backgroundColor: isDark
                      ? "rgba(255,255,255,0.03)"
                      : "rgba(255,255,255,0.6)",
                    padding: 10,
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <T
                      v="caption"
                      numberOfLines={2}
                      style={{ fontSize: 9, color: d.faint, marginTop: 2 }}
                    >
                      {it.sub}
                    </T>
                  </View>
              ...
              it.arabic ? (
                <T
                  v="caption"
                  style={{ fontSize: 9.5, color: d.faint, marginBottom: 2 }}
                >
                  {it.title}
                </T>
              ) : null}
              ...
              it.kind === "group" ? (
                ...
                >
                  {it.kind === "reel"
                    ? "Reel shared from Videos"
                    : it.kind === "quiz"
                      ? "Quiz score shared from Islamic Quiz"
                      : it.kind === "riddle"
                        ? "Riddle shared from Islamic Riddles"
                        : it.kind === "group"
                          ? "Group invite — tap to open and join"
                          : "Post shared from the community feed"}
                </T>
              ) : null}
              <View
                style={{
    ...
    return (
      ...
      <Animated.View key={m.rk ?? m.id} style={{ opacity: isFocus ? 1 : dim }}>
        <BubbleIn mine={mine} animate={freshIds.current.has(m.id)}>
          ...
          >
            <SwipeReply
              onReply={() => openReply(m.id)}
              tint={isDark ? "#4AE38F" : "#1D6F42"}
              style={{ maxWidth: "76%" }}
            >
              <Pressable
                ref={(r) => {
                  rowRefs.current[m.id] = r as never;
                }}
              ...
              >
                  ...
                  >
                    ...
                    <T
                      ...
                      style={{
                        fontSize: 10,
                        lineHeight: 14,
                        color: mine ? "rgba(255,255,255,0.85)" : d.subtext,
                        marginTop: 1,
                      }}
                    >
                      {m.reply.text || "Message deleted"}
                    </T>
                  </View>
                ) : null}
                {m.deleted ? (
                  <T
                    ...
                    style={{
                      ...
                      lineHeight: 18,
                      fontStyle: "italic",
                      color: mine ? "rgba(255,255,255,0.8)" : d.faint,
                    }}
                  >
                    Message deleted
                  </T>
                ...
                {!m.deleted && findUrl(m.text) ? (
                  <LinkPreviewCard
                    url={findUrl(m.text) as string}
                    dark={mine ? true : isDark}
                    compact
                  />
                ) : null}
                <View
                ...
                >
                  <T
                    ...
                    style={{
                      fontSize: 8.5,
                      color: mine ? "rgba(255,255,255,0.7)" : d.faint,
                    }}
                  >
                    {m.ago}
                  </T>
                  {reaction ? (
                    <PopEmoji emoji={reaction} size={11} />
                  ) : peer ? (
                    <PopEmoji emoji={peer} size={11} />
                  ) : null}
  ...
  const focusRow: ChatMsg | ShareItem | null =
    !thread || !focus
      ? null
      : focus.kind === "msg"
  ...
  const menuGeom = focusPos
    ? (() => {
        const rows =
          3 +
          (focusMine && !deletedRow && focus?.kind === "msg" ? 1 : 0) +
          (focusMine && !deletedRow ? 1 : 0);
        ...
        if (top + estH > SCREEN_H - 10) {
          top = Math.max(10, focusPos.y - estH - 10);
        }
        return { left, top };
      })()
    : null;
  ...
  const body = (
    <KeyboardAvoidingView
      ...
      style={{ flex: 1, backgroundColor: isDark ? "#07100C" : "#F6FAF7" }}
    >
      {/* header */}
      <View
        style={{
      ...
      >
        <Pressable
          onPress={() => (thread ? setOpenFriend(null) : onClose())}
          hitSlop={10}
          style={{
            width: 36,
            height: 36,
        ...
        {thread ? (
          <View>
            <Pressable
              onPress={() => {
                /* pass 83-21 — the blocked viewer cannot open the blocker's profile */ if (
                  blockFlags[thread.friend]?.by
                ) {
                  return;
                }
                /* pass 83-14 — remember the chat, then HIDE the inbox so the profile doesn't render under it; the community screen reopens this exact thread on focus */ if (
                  !standalone
                ) {
                  storage
                    .setItem("dl_inbox_reopen", thread.friend)
                    .catch(() => {});
                  onNavigateAway?.();
                }
                router.push(`/profile/${thread.friend}` as never);
              }}
              hitSlop={6}
            >
              <AvatarImage
                source={acc(thread.friend).photo ?? null}
                name={dispName(thread.friend)}
                size={38}
                tint="rgba(46,204,113,0.2)"
                border={d.cardBorder}
              />
            ...
            {blockFlags[thread.friend]?.by ? null : (
              <View
                style={{
                  ...
                  height: 11,
                  borderRadius: 6,
                  backgroundColor: isOnline(thread.friend)
                    ? "#2ECC71"
                    : "#E05252",
                  borderWidth: 2,
                  borderColor: d.card,
        ...
        <View style={{ flex: 1, minWidth: 0 }}>
          <Pressable
            onPress={() => {
              if (thread) {
                if (blockFlags[thread.friend]?.by) {
                  return;
                }
                /* pass 83-21 */ if (!standalone) {
                  storage
                    .setItem("dl_inbox_reopen", thread.friend)
                    .catch(() => {});
                  onNavigateAway?.();
          ...
          >
            ...
            <T
              ...
              numberOfLines={1}
              ellipsizeMode="tail"
              style={{ fontWeight: "800", fontSize: 17, color: d.text }}
            >
            ...
            >
              {thread
                ? outRequests.has(thread.friend)
                  ? `Message request · 3-message limit until @${thread.friend} accepts`
                  : isOnline(thread.friend)
                    ? "Online now"
                    : seenMap[thread.friend]
                      ? `Last seen ${lastSeenText(String(seenMap[thread.friend]))}`
                      : `@${thread.friend}`
                : "Reels, posts, duas & ayahs shared with you"}
            </T>
          </Pressable>
        </View>
      ...
      {requestsOpen && !thread ? (
        ...
        >
          ...
          >
            ...
            >
              ...
              <T v="caption" style={{ fontWeight: "800", color: d.text }}>
                Back
              </T>
            </Pressable>
            <T
          ...
          {Object.keys(reqMap).length === 0 ? (
            <T
              v="bodyS"
              style={{ color: d.faint, textAlign: "center", marginTop: 36 }}
            >
              No pending requests.
            </T>
          ) : (
                ...
                >
                  ...
                  <View style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <T
                      v="caption"
                      numberOfLines={1}
                      style={{ fontSize: 10.5, color: d.faint, marginTop: 1 }}
                    >
                      @{uname} · wants to message you
                    </T>
                  </View>
                </View>
                <View style={{ flexDirection: "row", gap: 8, marginTop: 11 }}>
                  <Pressable
                    disabled={reqBusy === uname}
                    onPress={() => void actOnRequest(uname, "accept")}
                    style={{
                      flex: 1,
                      flexDirection: "row",
                      alignItems: "center",
                      ...
                      paddingVertical: 10,
                      opacity: reqBusy === uname ? 0.6 : 1,
                    }}
                  >
                    {reqBusy === uname ? (
                      <ActivityIndicator size="small" color="#062312" />
                    ) : (
                      <FontAwesome5 name="check" size={11} color="#062312" />
                    )}
                    <T
                      v="caption"
                      style={{
                  ...
                  <Pressable
                    disabled={reqBusy === uname}
                    onPress={() => void actOnRequest(uname, "block")}
                    style={{
                      borderRadius: 11,
                      borderWidth: 1,
                      borderColor: d.cardBorder,
                  ...
                  <Pressable
                    ...
                    onPress={() =>
                      Alert.alert(
                        "Report this account?",
                        "We will review their messages and account.",
                        [
                          { text: "Cancel", style: "cancel" },
                          {
                            text: "Report",
                            style: "destructive",
                            onPress: () => void actOnRequest(uname, "report"),
                          },
                        ],
                      )
                    }
          ...
          >
            ...
            {Object.keys(reqMap).length > 0 ? (
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  <T
                    v="bodyS"
                    style={{ fontWeight: "800", fontSize: 13.5, color: d.text }}
                  >
                    Message requests
                  </T>
                  <T
                    ...
                    style={{ fontSize: 10.5, color: d.faint, marginTop: 1 }}
                  >
                    {Object.keys(reqMap).length} person
                    {Object.keys(reqMap).length > 1 ? "s" : ""} waiting · tap to
                    review
                  </T>
                </View>
                ...
                return (
                  <Pressable
                    ...
                    onPress={() => {
                      ...
                      setUnreadMap((prev) =>
                        prev[t.friend] ? { ...prev, [t.friend]: 0 } : prev,
                      );
                    }}
                    style={({ pressed }) => ({
                      flexDirection: "row",
                  ...
                  >
                    ...
                    <View style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {t.blocked ? (
                          <View
                            style={{
                              borderRadius: 7,
                              borderWidth: 1,
                              borderColor: "rgba(224,82,82,0.55)",
                      ...
                      <T
                        v="caption"
                        numberOfLines={1}
                        style={{ color: d.faint, fontSize: 10.5, marginTop: 2 }}
                      >
                        {(() => {
            ...
            Object.keys(reqMap).length === 0 ? (
              ...
              >
                ...
                <T
                  v="body"
                  style={{ fontWeight: "800", fontSize: 14, color: d.text }}
                >
                  No messages yet
                </T>
                <T
                ...
                >
                  Open someone's profile and tap Message to start a chat. Until
                  they accept, your chat waits in their Message requests.
                </T>
                {sugg.length > 0 ? (
                  <View style={{ marginTop: 22, alignSelf: "stretch" }}>
                    ...
                    >
                      YOU FOLLOW EACH OTHER — SAY SALAM
                    </T>
                    {sugg.map((sg) => (
                      <Pressable
                        key={sg.username}
                      ...
                      >
                        ...
                        <View
                          style={{
                            borderRadius: 9,
                            backgroundColor: isDark ? "#1D6F42" : "#2ECC71",
                            paddingHorizontal: 12,
                            paddingVertical: 7,
                          }}
                        >
          ...
          >
            ...
            >
              ...
              >
                <T
                  v="body"
                  style={{ fontWeight: "800", fontSize: 15, color: d.text }}
                >
                  New message
                </T>
                <T
                ...
                >
                  Pick a mutual follow — or search above for anyone.
                </T>
                <ScrollView showsVerticalScrollIndicator={false}>
                  {sugg.length === 0 ? (
                    ...
                    >
                      No mutual follows yet — follow someone and they'll appear
                      here.
                    </T>
                  ) : (
                    sugg.map((sg) => (
        ...
        >
          {histLoading && flow.length === 0 ? (
            <BreathingMessages dark={isDark} />
          ) : null}
          {flow.map((row) =>
            row.kind === "share"
          ...
          >
            Double-tap to react · shares are in-app content only
          </T>
        </ScrollView>
      )}
      ...
      {thread ? (
        <View
          ...
          style={{
            paddingHorizontal: 12,
            paddingTop: 8,
            paddingBottom: kbOpen ? 8 : Math.max(insets.bottom, 12),
            borderTopWidth: 1,
            borderTopColor: isDark
              ? "rgba(255,255,255,0.08)"
              : "rgba(20,36,28,0.08)",
            backgroundColor: isDark ? "#07100C" : "#F6FAF7",
            gap: 8,
          }}
        >
          <ScrollView
            ...
            {(
                ...
                onPress={() =>
                  shareBack(
                    ...
                    k === "ayah"
                      ...
                      : k === "hadith"
                        ? "Muslim 2568 — visit the sick"
                        : k === "dua"
                          ? "Dua for guidance"
                          : k === "post"
                            ? "A post you might love"
                            : k === "profile"
                              ? "Profile — Ustādh Ibrāhīm"
              ...
              >
                <FontAwesome5
                  name={ic as never}
                  size={9}
                  color={KIND_META[k].tint}
                />
                <T
                  v="caption"
                  style={{ fontSize: 9.5, fontWeight: "700", color: d.subtext }}
                >
                  {label}
                </T>
              </Pressable>
            ...
            >
              ...
              <View style={{ flex: 1, minWidth: 0 }}>
                ...
                <T
                  v="caption"
                  numberOfLines={1}
                  style={{ fontSize: 10.5, color: d.subtext, marginTop: 1 }}
                >
                  {replyTo.text}
                </T>
              </View>
      ...
      {menu && thread ? (
        <>
          ...
          <View
            style={{
              position: "absolute",
              top: (standalone ? insets.top + 8 : 0) + 52,
              right: 14,
              zIndex: 50,
              width: 196,
              borderRadius: 14,
              ...
              [
                ...
                /* pass 83-14 — unblock right here; settings used to be the only way out */
                {
                  ...
                  label:
                    ...
                    acc(thread.friend).full_name,
                  icon: blocked ? "unlock" : "user-slash",
                  color: blocked ? (isDark ? "#4AE38F" : "#0E7A46") : "#E05252",
                },
              ] as const
            ).map((it2) => (
              <Pressable
                  ...
                  if (blocked) {
                    ...
                    void blockUser(who, false).then((ok) => {
                      if (ok) {
                        ...
                        setBlockFlags((prev) => ({
                          ...prev,
                          [who]: { b: false, by: prev[who]?.by ?? false },
                        }));
                        setThreads((prev) =>
                          prev.map((t) =>
        ...
        >
          ...
          >
            ...
            <T
              ...
              style={{ fontWeight: "800", fontSize: 17, color: d.text }}
            >
              {thread ? `Report ${acc(thread.friend).full_name}` : "Report"}
            </T>
            <T
              v="caption"
              style={{ color: d.faint, marginTop: 3, marginBottom: 14 }}
            >
              ...
              return (
                <Pressable
                  ...
                  style={{
                    ...
                    borderRadius: 13,
                    borderWidth: 1.5,
                    borderColor: on ? "#E05252" : d.cardBorder,
                    backgroundColor: on ? "rgba(224,82,82,0.1)" : d.bg,
                    paddingHorizontal: 13,
                    paddingVertical: 12,
                    marginBottom: 8,
                  }}
            ...
            <Pressable
              ...
              style={{
                marginTop: 14,
                borderRadius: 14,
                backgroundColor: reportType ? "#E05252" : d.cardBorder,
                paddingVertical: 14,
                alignItems: "center",
                opacity: reportType ? 1 : 0.6,
              }}
            >
              <T
                v="bodyS"
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {thread ? `Block ${acc(thread.friend).full_name}?` : "Block?"}
            </T>
            <T
              v="caption"
              style={{
            ...
            >
              They will not be able to message you, and this conversation will
              be hidden from your inbox.
            </T>
            <View
              style={{
                flexDirection: "row",
            ...
            >
              ...
              >
                <T
                  v="bodyS"
                  style={{ fontWeight: "700", fontSize: 13, color: d.text }}
                >
                  Cancel
                </T>
              </Pressable>
                  ...
                  {focusMode === "menu" ? (
                    <>
                      ...
                      <MenuRow
                        ...
                        color={d.text}
                        line={glassLine}
                        onPress={() => startForward(focus.id, focus.kind)}
                      />
                      <MenuRow
                        icon={copied ? "check" : "copy"}
                        label={copied ? "Copied ✓" : "Copy"}
                        color={copied ? "#4AE38F" : d.text}
                        line={glassLine}
                        onPress={() => copyRow(focus.id)}
                      />
                      ...
                      {focusMine && !deletedRow ? (
                        <MenuRow
                          ...
                          color="#E05252"
                          line={glassLine}
                          onPress={() => deleteRow(focus.id, focus.kind)}
                        />
                      ) : null}
                    </>
                  ) : (
                    ...
                    <View style={{ paddingVertical: 6 }}>
                      ...
                      >
                        ...
                        <T
                          v="bodyS"
                          style={{ flex: 1, fontSize: 12.5, color: d.text }}
                        >
                          Delivered
                        </T>
                        <T v="caption" style={{ fontSize: 11, color: d.faint }}>
                          {(focusMsg?.createdAt || "").slice(11, 16) || "—"}
                        </T>
                      </View>
                      <View
                        style={{
                      ...
                      >
                        <FontAwesome5
                          name="check-double"
                          size={12}
                          color={focusMsg?.readAt ? "#4AE38F" : d.faint}
                        />
                        <T
                          v="bodyS"
                          style={{ flex: 1, fontSize: 12.5, color: d.text }}
                        >
                          {focusMsg?.readAt ? "Seen" : "Not seen yet"}
                        </T>
                        <T v="caption" style={{ fontSize: 11, color: d.faint }}>
                          {focusMsg?.readAt
                            ? String(focusMsg.readAt).slice(11, 16)
                            : ""}
                        </T>
                      </View>
                    </View>
      ...
      <Modal
        ...
        transparent
        animationType="slide"
        onRequestClose={() => setForward(null)}
      >
        <View
          style={{
            flex: 1,
        ...
        >
          ...
          >
            ...
            <View style={{ flex: 1, minWidth: 0 }}>
              <T
                v="h2"
                numberOfLines={1}
                style={{ fontSize: 16, fontWeight: "800", color: d.text }}
              >
                Forward message
              </T>
              <T
                v="caption"
                numberOfLines={1}
                style={{ fontSize: 10, color: d.faint }}
              >
                {forwardPicked.size} selected
              </T>
            </View>
          </View>
          <T
          ...
          <ScrollView
            showsVerticalScrollIndicator={false}
            contentContainerStyle={{ paddingHorizontal: 12, paddingBottom: 92 }}
          >
              ...
              return (
                <Pressable
                  ...
                  onPress={() => {
                    ...
                    setForwardPicked((p) => {
                      const n = new Set(p);
                      if (n.has(a.username)) n.delete(a.username);
                      else n.add(a.username);
                      return n;
                    });
                  }}
                  style={{
                ...
                >
                  ...
                  <View style={{ flex: 1, minWidth: 0 }}>
                    <T
                      v="bodyS"
                      numberOfLines={1}
                      style={{ fontSize: 13, fontWeight: "700", color: d.text }}
                    >
                      {a.full_name}
                    </T>
                    <T
                  ...
                  <View
                    style={{
                      ...
                      borderRadius: 11,
                      borderWidth: 1.6,
                      borderColor: on ? "#1F8F5C" : d.cardBorder,
                      backgroundColor: on ? "#1F8F5C" : "transparent",
                      alignItems: "center",
                      justifyContent: "center",
                    }}
                  >
          ...
          >
            <Pressable
              disabled={!forwardPicked.size}
              onPress={() => doForwardMany([...forwardPicked])}
              style={{
                flexDirection: "row",
                alignItems: "center",
                justifyContent: "center",
                gap: 8,
                borderRadius: 14,
                backgroundColor: forwardPicked.size ? "#1F8F5C" : d.cardBorder,
                paddingVertical: 13,
                opacity: forwardPicked.size ? 1 : 0.5,
              }}
            >
              <FontAwesome5 name="share" size={12} color="#fff" />
              <T
                v="bodyS"
                style={{ fontSize: 13.5, fontWeight: "800", color: "#fff" }}
              >
                Forward{forwardPicked.size ? ` to ${forwardPicked.size}` : ""}
              </T>
            </Pressable>
          </View>
        </View>
  ...
  return (
    <Modal
      ...
      transparent
      animationType="slide"
      onRequestClose={() => (thread ? setOpenFriend(null) : onClose())}
    >
      {body}
    </Modal>
  );
