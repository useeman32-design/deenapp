...
- Course certificate implementation: `/api/courses/quiz_complete.php` records attempts; a certificate is issued only after all required lessons and at least 80% on the quiz. Certificate payload now includes quiz score/total/percentage and an absolute QR verification URL. Native `courses.tsx` renders the DeenLink certificate card and QR using the `qrcode` package and configured signature metadata. Admin Course Builder has an `Add 20 starter courses` action that creates/refreshes 20 curated course records with three editable lessons each; admin can then expand or replace content and upload signature/template assets. Group join-request management was already live in `group.tsx` with Accept/Decline and server endpoints; authenticated end-to-end verification remains pending.

- Certificate full-page/export pass: native course results now open a full certificate view. `expo-file-system/legacy` writes a complete SVG certificate, `expo-sharing` exports it, and `expo-media-library` requests permission and attempts gallery saving. `qrcode.d.ts` now declares the QR SVG Promise API. Courses-specific TypeScript checks pass; remaining tsc failures are pre-existing shop `image_url` typing and fatwa source comparison. PHP/Docker are unavailable locally, so PHP lint must run in CI or the deployment host.

- Certificate export now follows the same proven app path as Share as Image: `SvgXml` certificate ref → `shareSvgRef` / `saveSvgRefAsJpg` from `src/lib/svgExport.ts` → PNG data → JPEG via `expo-image-manipulator` → `expo-sharing` or `expo-media-library`. This replaces the prior raw SVG file export, which was not reliably gallery-compatible on Android/iOS. A new native build/OTA is still required to deliver the change.

- Important implementation detail: certificate full-page export uses a forwardRef native `Svg` tree, not `SvgXml` (which cannot reliably accept refs). The `CertificateSvg` component renders the certificate and QR matrix directly, so `svgRefToPng` can rasterize it on Android/iOS.

- Social next pass implementation: `src/lib/postEvents.ts` broadcasts post deletion across mounted home/community/profile/group surfaces. Notifications pass `comment`/`reply` targets to `/tools/post`, which opens CommentsModal and highlights the matching item. Profile links use `/profile/[username]` params and strip leading @. `FeedCard` YouTube iframes use IntersectionObserver and unmount below 28% visibility; native video focus/AppState stop logic remains. Post share image metadata now carries author name, @username, time, content, likes and comments with a fixed-height SVG card. Quiz mojibake strings were repaired. Admin report list labels `user_type=guest` reporters as Guest.

- Testing pass changes: admin `pageshow` now hides the shared loader on browser back/forward; Boost and Learning Hub layouts are more balanced; follower notification mapping avoids duplicated names and explicitly says `New followers`; blocking a message request updates both the conversation request and canonical blocked-user list endpoint. Do not claim Shop CRUD/variants/order badge, reciter image research, Mushaf/Compass paid themes, or full server-authored course/test CRUD complete yet; these need the next implementation pass.

- Latest remaining-pass changes: removed mobile AI provider key/web-search/Cloud AI controls in favor of secure server-managed AI capability controls; backend already has Gemini fallback and provider monitoring. Reciter rows now expose `photo_url` and the reciter picker renders portraits (stable generated fallback until curated portraits are uploaded). Do not mark Shop CRUD, Mushaf/Compass paid theme management, or server-authored course tests complete yet.
