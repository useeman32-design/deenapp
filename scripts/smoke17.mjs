...
// videos: Following/For you tabs, search, library, create, repost
{
  ...
  if (followingTab) {
    ...
    await page.waitForTimeout(1000);
    t = await bodyText();
    check('videos: following filters', t.includes('@alameen') && !t.includes('@Gimba'), '');
  ...
  const libBtn = await page.evaluate(() => {
    const els = Array.from(document.querySelectorAll('div'));
    const cands = els.filter((d) => d.children.length === 0 && (d.textContent || '').trim() === 'Saved');
    const vis = cands.filter((d) => { const r = d.getBoundingClientRect(); return r.y > 700 && r.x < 260; });
    if (!vis.length) return null;
    const r = vis[0].getBoundingClientRect();
    return { x: r.x + r.width / 2, y: r.y - 22 };
  });
  check('videos: library via bottom menu Saved', !!libBtn, libBtn ? 'found' : 'missing');
  if (libBtn) {
  ...
  const createBtn = await page.evaluate(() => {
    const els = Array.from(document.querySelectorAll('div'));
    const cands = els.filter((d) => d.children.length === 0 && (d.textContent || '').trim() === '\uf067');
    const vis = cands.filter((d) => { const r = d.getBoundingClientRect(); return r.y > 580 && r.x > 180; });
    if (!vis.length) return null;
    const r = vis[0].getBoundingClientRect();
    return { x: r.x + r.width / 2, y: r.y + r.height / 2 };
  });
...
// pass-14: back chevron, bottom pill, inline comments sheet
{
  ...
  const menu = t.includes('Inbox') && t.includes('Friends') && t.includes('Saved');
  check('videos: bottom menu Inbox·Friends·Saved', menu, menu ? 'ok' : t.slice(0, 90).replace(/\n/g, ' '));
  const rp = t.includes('Reposted by');
  check('videos: reposted-by pill', rp, rp ? 'ok' : 'none in view');
...
// pass-15: center circle tab, quran daily cards + progress hero + reader audio, hadith collections, profile identity
{
  ...
  const toolsLabel = await hitPoint('Array.from(document.querySelectorAll("div")).filter((d) => d.children.length === 0 && (d.textContent || "").trim() === "Worship" && d.getBoundingClientRect().y > 640)');
  check('tabs: tools slot is a normal tab', !!toolsLabel, toolsLabel ? 'label' : 'missing');
  if (toolsLabel) {
    await page.mouse.click(toolsLabel.x, toolsLabel.y);
    await page.waitForTimeout(2400);
  ...
  if (cont) {
    ...
    check('quran reader: opens + audio bar', rTxt.includes('Alafasy') && rTxt.includes('Ayah'), rTxt.slice(0, 70).replace(/\n/g, ' '));
    check('quran reader: mushaf toggle', rTxt.includes('Mushaf'), '');
    check('quran reader: /read route', page.url().includes('/read'), page.url().slice(-24));
...
// pass-17: inbox screen, friends tab, avatar preview, settings screen, dua data, live reader text
{
  await page.goto(BASE + '/', { waitUntil: 'domcontentloaded' });
  await page.waitForTimeout(3000);
  const chip = await hitPoint('Array.from(document.querySelectorAll("div")).filter((d) => d.children.length === 0 && (d.textContent || "").trim() === "Videos")');
  if (chip) { await page.mouse.click(chip.x, chip.y); await page.waitForTimeout(3000); }
  // friends 3rd tab in segmented
  let t = await bodyText();
  check('videos: Friends tab in segmented', t.includes('Friends'), '');
  // inbox full screen
  const inboxBtn = await hitPoint('Array.from(document.querySelectorAll("div")).filter((d) => d.children.length === 0 && (d.textContent || "").trim() === "Inbox" && d.getBoundingClientRect().y > 640)');
  if (inboxBtn) {
    await page.mouse.click(inboxBtn.x, inboxBtn.y);
    await page.waitForTimeout(1200);
    t = await bodyText();
    check('videos: inbox full-screen shares', t.includes('shared a video with you') && t.includes('Chat is off for shares'), t.slice(0, 90).replace(/\n/g, ' '));
    await page.screenshot({ path: SHOTS + '/p17-inbox.png' });
    const back = await page.evaluate(() => { const els = Array.from(document.querySelectorAll('div')); const el = els.filter((d) => d.children.length === 0 && (d.textContent || '').trim() === '\uf053'); return el.length ? (() => { const r = el[el.length - 1].getBoundingClientRect(); return { x: r.x + r.width / 2, y: r.y + r.height / 2 }; })() : null; });
    if (back) { await page.mouse.click(back.x, back.y); await page.waitForTimeout(800); }
  } else check('videos: inbox full-screen shares', false, 'inbox btn missing');
  // avatar preview (tap the reel avatar — left-bottom area)
  const av = await page.evaluate(() => { const v = document.querySelector('video'); if (!v) return null; return { x: 33, y: v.getBoundingClientRect().bottom - 192 }; });
  if (av) {
    await page.mouse.click(av.x, av.y);
    await page.waitForTimeout(900);
    t = await bodyText();
    check('videos: avatar preview w/ deenlink tag', t.includes('@deenlink'), t.slice(0, 80).replace(/\n/g, ' '));
    await page.screenshot({ path: SHOTS + '/p17-avatar-preview.png' });
    await page.mouse.click(195, 150);
    await page.waitForTimeout(600);
  }
  ...
  if (setTab) {
    await page.waitForTimeout(1400);
    t = await bodyText();
    check('settings: screen opens w/ rows', t.includes('Appearance') && t.includes('Sign out') && t.includes('Privacy'), page.url().slice(-24));
    await page.screenshot({ path: SHOTS + '/p17-settings.png' });
  } else check('settings: screen opens w/ rows', false, 'tab missing');
  // duas data
  await page.goto(BASE + '/tools/dua', { waitUntil: 'domcontentloaded' });
  await page.waitForTimeout(2200);
  t = await bodyText();
  check('dua: dataset renders', t.includes('Sayyidul Istighfar') || t.includes('Morning'), t.slice(0, 80).replace(/\n/g, ' '));
  // reader live text (v1 api) — navigate via hub (deep link 404s on serve)
  await page.goto(BASE + '/quran', { waitUntil: 'domcontentloaded' });
  await page.waitForTimeout(2600);
  const cont17 = await page.evaluate(() => { const els = Array.from(document.querySelectorAll('div')); const el = els.filter((d) => d.children.length === 0 && (d.textContent || '').trim() === 'Continue Reading'); if (!el.length) return null; const r = el[0].getBoundingClientRect(); return { x: r.x + r.width / 2, y: r.y + r.height / 2 }; });
  if (cont17) { await page.mouse.click(cont17.x, cont17.y); }
  await page.waitForTimeout(4500);
  t = await bodyText();
  check('reader: live text via v1 api', t.includes('alquran.cloud') || t.includes('Ash-Sharh'), t.slice(0, 90).replace(/\n/g, ' '));
  // mushaf loads
  const mushafBtn = await hitPoint('Array.from(document.querySelectorAll("div")).filter((d) => d.children.length === 0 && (d.textContent || "").trim() === "Mushaf")');
  if (mushafBtn) {
    await page.mouse.click(mushafBtn.x, mushafBtn.y);
    await page.waitForTimeout(3500);
    t = await bodyText();
    check('reader: mushaf page loads', t.includes('/ 604'), t.slice(0, 90).replace(/\n/g, ' '));
    await page.screenshot({ path: SHOTS + '/p17-mushaf.png' });
  }
  // quran hub: daily cards BELOW the big cards
  await page.goto(BASE + '/quran', { waitUntil: 'domcontentloaded' });
  await page.waitForTimeout(2600);
  const order = await page.evaluate(() => {
    const t = document.body.innerText;
    return { big: t.indexOf('Hadith Collections'), daily: t.indexOf('DAILY AYAH'), seerah: t.includes('Seerah') };
  });
  check('quran hub: daily cards below big cards', order.big > 0 && order.daily > order.big, JSON.stringify(order));
}
