...
// videos feed opens from Home → Quick Access (first button)
{
  ...
  const rail = await page.evaluate(() => {
    ...
    return { like: t.includes('1,284'), comment: t.includes('96'), save: t.includes('Download'), share: t.includes('Share'), music: t.includes('nasheed') };
  });
...
// videos: Following/For you tabs, search, library, create, repost
{
  ...
  const libBtn = await page.evaluate(() => {
    const els = Array.from(document.querySelectorAll('div'));
    const cands = els.filter((d) => d.children.length === 0 && (d.textContent || '').trim() === '\uf02e');
    const vis = cands.filter((d) => { const r = d.getBoundingClientRect(); return r.y > 5 && r.y < 130; });
    if (!vis.length) return null;
    const r = vis[0].getBoundingClientRect();
    return { x: r.x + r.width / 2, y: r.y + r.height / 2 };
  });
  check('videos: library button', !!libBtn, libBtn ? 'found' : 'missing');
  if (libBtn) {
  ...
  // repost
  const repostBtn = await page.evaluate(() => {
    const els = Array.from(document.querySelectorAll('div'));
    const cands = els.filter((d) => d.children.length === 0 && (d.textContent || '').trim() === 'Repost');
    const vis = cands.filter((d) => { const r = d.getBoundingClientRect(); return r.y > 300; });
    if (!vis.length) return null;
    const r = vis[0].getBoundingClientRect();
    return { x: r.x + r.width / 2, y: r.y - 30 };
  });
  check('videos: repost button', !!repostBtn, repostBtn ? 'found' : 'missing');
  if (repostBtn) {
    await page.mouse.click(repostBtn.x, repostBtn.y);
    await page.waitForTimeout(900);
    t = await bodyText();
    check('videos: repost toast', t.includes('Reposted'), t.includes('Reposted') ? 'ok' : 'no toast');
    await page.screenshot({ path: '/tmp/p13-videos-repost.png' });
  }
  // create studio
