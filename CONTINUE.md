...
- Lost-pass recovery: Inbox previews now use the server conversation last message (`last_body`) and loaded chat/share content instead of showing a shared-item count fallback. SunPath now holds the moon at Isha after the final prayer instead of retracing the arc. Tasbeeh uses a 66-bead ring with an attached head/connector. Daily Zikr group headings use icons instead of emojis, and athkar detail text is constrained inside a scrollable modal. Morning/evening declarations and Ayat al-Kursi fallback text were completed. Fatwa sources no longer show Ask a Scholar. Charity no longer displays the DeenPoints balance chip. Privacy now exposes the server-backed Allow group adding switch through `/api/users/group_privacy.php`.

- Inbox/DM header now always includes the native safe-area top inset, including the community modal variant, so it cannot merge into the status bar.

- Courses pass: added a protected admin action to seed a curated 20-course starter catalogue with three editable lessons per course. Courses use the existing admin-managed database and learner flow. Added server quiz-attempt recording with an 80% pass threshold; certificates now require completed lessons plus a passed quiz. Native course results show a DeenLink certificate card with learner full name, course, percentage, issue date, configured signature, logo, and QR verification code. Group-admin join-request accept/reject UI and backend were already present; end-to-end authenticated verification remains pending.

- Certificate presentation pass: added a dedicated full certificate modal, generated a complete DeenLink SVG certificate artifact, Share/Export via expo-sharing, and Save to Gallery via expo-media-library. The SVG includes title, learner, course, percentage, date, signature and QR verification code. TypeScript errors in courses.tsx are cleared; the repository still has unrelated pre-existing errors in shop/index.tsx and tools/fatwa.tsx. PHP syntax validation remains unavailable because neither PHP nor Docker is installed in this workspace.

- Cross-platform export audit: the existing app's `src/lib/svgExport.ts` is the canonical native flow: render react-native-svg, call `toDataURL`, convert PNG to JPEG with expo-image-manipulator, then expo-sharing or expo-media-library. Certificate export was changed to use that same flow through an exportable SvgXml ref; it no longer shares/saves raw SVG. This is the Android/iOS-compatible path used by ContentShareSheet. Courses-specific TypeScript checks pass; full-project errors remain unrelated shop/fatwa typing issues.

- Final export correction: `SvgXml` is a function component and does not reliably accept an export ref. The certificate full page now uses a real forwardRef react-native-svg `Svg` tree (`CertificateSvg`), matching the existing ShareCardSvg/native export pattern. This ensures `toDataURL` is available on Android and iOS before PNG→JPEG conversion.

- Next-pass social fixes: added a cross-surface post deletion event bus so successful deletes from group/community/home/profile immediately remove the same post from every mounted surface. Notification post/comment taps now route to the single-post view and open the comments sheet with the target comment highlighted, including nested replies. Profile navigation now uses Expo Router dynamic params and normalizes @ usernames. Native/local video focus behavior remains strict; web YouTube post iframes now unmount when scrolled below the visibility threshold so playback stops. Guest greeting fallback is now Guest. Repaired mojibake in the quiz dataset, aligned profile FeedCard dash styling with community, and redesigned post share cards with author/username/time/content/like/comment metadata and fixed 1350px image height.

- Notification routing correction: server notification metadata already carries `post_id` and `comment_id`; the native `NotifRow` mapper now preserves metadata/postId so comment and nested-comment notifications open the correct post rather than treating the comment id as the post id.
...
## Pass 87 (2026-09-15)
- Group privacy: open_join now persists on FLICK (not just Save) + reverts with a clear alert on failure; join requests create an owner notification (user_notifications row) and approvals notify the requester. Server-side update.php + requests queue confirmed live.
- Courses: Course Quizzes page renamed Course Tests; new idempotent seeder api/admin/courses/seed_pass87.php ("Fill catalogue" button) creates 20 NEW courses (10 lessons each), tops every course to 10 lessons, seeds a 20-question test bank per course (never overwrites edited banks). Data files pass87_data_{a1,a2,b1,b2}.php.
- Shop: shipping type (free/paid + cost, NGN→USD at save), per-category SKU auto-gen (CAT-0001), featured image auto-sync from first upload + reorder sync, admin cards show the image, variant images via dropdown (no more Media-ID typing), dawah… n/a; save no longer blocked for missing images. Checkout adds shipping once per product line; orders return can_mark_received + new /api/shop/confirm_received.php (user "Mark as Received" w/ confirm dialog).
- Admin chrome: sidebar.js wires theme/menu/profile on EVERY page + bfcache-back loader fix; uploads-kit (progress bar, XHR upload, searchable module picker used by campaigns + announcements); campaigns: upload w/ progress, list errors visible + retry, bigger previews; announcements: save now shows real upload progress; megaphone→bullhorn; Course Tests in nav; standalone notifications pages REMOVED (bell dropdown fixed instead: tap-toggle, mobile-safe centered panel, rows deep-link reports.html?report=N, Show more → reports; reports.html supports ?report/?status/?type/?q with highlight+scroll).
- AI: chat endpoint rewritten to provider fallback chain Groq→Gemini→OpenRouter→HF with per-attempt logging (ai_query_logs + ai_provider_logs feed the existing AI monitor page); keys managed in admin. App AI page: API-key field, settings entry and web-search toggle removed; answer mode replaced by Quick/Deep reasoning capability.
- Scholars: applications now upload to server (api/auth/scholar_apply.php, proof OR letter required, dawah links removed/optional) and land in the admin pending queue; approval/rejection sends EMAIL with level+aqeedah + in-app notification; public answers already auto-post to profile.
- App: FeedCard embeds unmount on navigation blur (strict stop); reels pause when the screen loses focus; suggestion strips capped (≤2+1); profile videos feed now refetches on return (no more empty→10s); share sheet: "Share as post" returns as a REAL post-card image (avatar/name/handle/time/content/♥💬) for text-only posts; posts with images/video/YouTube cannot share as image at all; shop cards + product page show shipping chips; "Save X%" replaces "-X%" badges; shop badges refresh on every return; scholars get a Desk button in Ask-Scholars header + profile My-Questions chip w/ unread badge; register: special-char password rule + scholar app reaches server; reciters sheet dedupes server rows vs local ones (real photos win) + Yasser Al-Dosari portrait added.
- Fixed: toggle_follow notification title "New Follower" (was name + @handle double); chat request BLOCK now writes user_blocks (shows in Blocked accounts).
- Live after owner pull: root entry-6e01e668348ccdc6cb2179cdc8e8e8b7 (dlapi main); gh-pages rebuilt.

## Pass 88 (2026-09-16) — launch-readiness pass: 11 owner reports closed, 3 server bugs found by RUNNING them
SPEED NOTE FROM OWNER: this pass took ~8 h and that is unacceptable. Work in big
batches, verify by executing (never re-derive closed leads), one status update at the
end. The closed-lead list below is the difference between 20 min and 3 h.

### What shipped (all verified by execution, not by reading code)
1. **Comment modal crash** — hook-order bug in CommentsModal (a hook after an early return). tsc clean, in the shipped bundle.
2. **Image-post crash** — NOT expo-image-picker itself and NOT a missing chunk: tailwind preflight was injected globally. `plugins/withNativeIcons.js` now sets `cssPlugins: ['@expo/vector-icons/css','expo-image-picker/css-plugins']` and every injection is wrapped as `{content:[{raw:css}],layer:'components'}`, so preflight is confined and author CSS wins. Verified in the live bundle: the WEB branch of `src/api/client.ts` shrinks via canvas/`URL.createObjectURL` and never imports `expo-image-manipulator` (that call now sits only in the native branch).
3. **Campaign banners** — owner: "the banners I set should be in the app, those were just demos". THREE defects, all found by running the endpoints against a real MySQL:
   • `api/admin/campaigns/save.php` INSERT reused the named placeholder `:cb` for created_by+updated_by; with `PDO::ATTR_EMULATE_PREPARES=false` (this app's setting) a named placeholder may be bound once → every NEW banner died `SQLSTATE[HY093] Invalid parameter number`.
   • same file validated `$data['status'] ?? 'active'` but STORED `$data['status']` → NULL into a NOT NULL enum → "Server error" whenever the form omitted status. Value is now computed once (`$statusClean`).
   • `api/campaigns/list.php` called `db_conn()`, which is NOT defined in `api/config/db.php` (only in the admin/courses common files); its catch-all swallowed the Error and returned `campaigns: []` forever. Now `DB::conn()`.
   • client `campaigns()` returns the ARRAY (even empty) on a successful call and `null` only when unreachable, and the home rail hides itself on an empty admin list instead of repainting the bundled demos.
   Verified: create → edit → delete → public rail returns the owner's banner.
4. **Reciter sheet** — one merged, scrollable list (no server+local duplicates), every row a real portrait (`assets/img/reciters/ghamdi.jpg` added), lock affordance uses `assets/img/deenpoints.png` (never an emoji/padlock), unlock calls `quranUnlockReciter` then `dp.sync()`.
5. **Mushaf themes + qibla compass designs locked behind DeenPoints** — new `api/themes/{common,list,unlock}.php`: catalogue priced in `common.php` (`dl_theme_catalogue()`), ownership in `user_settings.key='theme_locks'` + `user_theme_unlocks`, spend booked through the shared ledger helper. On a real DB: insufficient balance rejected with the price, free look ok, paid unlock wrote `deenpoints_ledger (user 115, delta −120, balance_after 380, event_type 'spend_theme', ref 'mushaf:sepia')`, double unlock = "Already unlocked" spent 0, list reflects ownership. Client: `src/lib/themeLocks.tsx` (optimistic spend + `dp.sync`, `ThemePriceChip`, gating in MushafPage + tools/qibla). NOTE: it is `.tsx`, not `.ts` — it contains JSX (tsc caught that).
6. **Emails: OTP code is primary, link is fallback** — `api/config/otp_email.php` gained `otp_email_html($code,$link='')` (button only when a link is passed) and `link_email_html($link,$note)`; `register.php` + `send_otp.php` send the code first, fall back to the link, and report `email_delivery`/`delivery` = `otp|link|none`; `resend_verification.php` deliberately stays link-based (the app polls `checkEmailVerified`). Verified on the rig: one email only, code in body, NO `<a href>`, `verify_otp` with the emailed code mints the session, wrong code rejected.
7. **Scholar signup** — the OTP field is now visible: `submitScholar` sets `otpDelivery`+`scholarNote` and reuses the same OtpVerify modal (gold application panel); `onVerified` → `adoptSession` else `login(otpEmail,lastPassword)`; nobody is signed in on cancel.
8. **Courses/tests depth** — `api/admin/courses/seed_pass88.php` (one click, idempotent, never rewrites an edited bank): tops EVERY published course to 12 lessons (curated content → the app's bundled curriculum → revision lessons generated from that course's own bank), merges curated/bundled questions up to 20 (dedupe on lowercased text), and backfills any `why` shorter than 40 chars with the answer + a lesson citation. Data lives in `api/admin/courses/data/pass88_thin_{a,b,c,d}.json` + `bundled_{curriculum,quizzes}.json` (regenerate the bundled pair with `node scripts/extract-bundled-courses.mjs <api>/api/admin/courses/data`). Verified: run 1 = 263 lessons / 540 questions / 509 explanations, runs 2-3 = 0/0/0; 47/47 courses ≥10 lessons; every course returns 20 questions from both `api/courses/quiz.php` and the admin editor.
   Lesson bodies are PLAIN `<p>` paragraphs only — `src/app/tools/courses.tsx` `stripHtml` converts `<br>`/`</p>` to newlines and deletes every other tag, so no h2/h3/ul.
9. **Quiz authoring parity** — `courses_quiz_sanitize($q,$rejectInsteadOfRepair=false)` REPAIRS rows (q|question|text, a|options|option1..6 incl. a JSON string, dedupe+cap 6, answer_index|correct|answer, why cap 900); the admin batch count runs each bank through the same function so the picker's number matches the editor. That is why every course now loads in Course Tests.
10. **Videos** — hub/tab sheets use the community card colour (`theme.dash.card` #0D1B13 dark / #FFFFFF light — NOT `dark.card`).
11. **Saved posts** — `src/lib/savedPosts.ts` `savedStore.swapId` (temp→real id on publish) + `useSavedTick()` for the profile; hydrate MERGES instead of clobbering in-flight saves; profile refetches on focus. Don't add a per-mount forced hydrate to `useSaved()`.
12. **Deep-link refresh went blank** — exported html carried `src="./_expo/…"`; a page at `/tools/courses.html` then asked for `/tools/_expo/…`. `scripts/absolutize-html.mjs` makes refs absolute and now takes the flavour prefix (root → `/_expo`, Pages → `/deenapp/_expo`), wired into BOTH `export-raw.sh` and `export-web.sh`. Verified live: `/deenapp/tools/courses.html` requests `/deenapp/_expo/…entry-28b2e494…js` → HTTP 200 (4.26 MB), while the old form 404s.
13. **Admin "Fill catalogue" lied** — it could die at the host's 30 s limit and report failure after doing most of the work. `seed_pass87.php` now sets `set_time_limit(0)`/512M and `admin/course-quizzes.html` runs both stages (87 then 88) with per-stage progress and real counts; the Course Tests picker lists ALL courses with true counts and says "uses the app's built-in quiz" when a bank is empty; the editor requires ≥40-char explanations.

### Gate results (this pass)
`tsc --noEmit` 0 errors · `php -l` clean on every touched PHP file (PHP binary is at /home/user/tools/php — `chmod +x` it, snapshots drop the bit; `scripts/*.sh` too) · `scripts/export-raw.sh` printed `ABSOLUTE-PASS OK` + `CHECK-RAW OK — 128 files, zero /deenapp/ refs` · 0 staged deletions in every commit · `git worktree` overlay for gh-pages with no `--delete` and no CNAME.

### Commits
deenapp@master `ffee4a8` (pass-88 app+scripts) → `f4b893d` (campaign rail) → `5f9c951` (absolutize prefix + rig). NOTE deenapp's default branch is **master** (a stray `main` was created and deleted — always `git ls-remote --symref origin HEAD` first).
deenlink-api@main `9e482c1` (themes endpoints + seeder + quiz parity + raw bundle entry-bca1571d) → `e5428d9` (campaign save/list/stamp fixes) → `01ab82a` (raw bundle entry-28b2e494).
...
### Closed leads — do not re-investigate (each cost an hour once)
• "Courses didn't update" is NOT cPanel cache, NOT the list endpoint, NOT seed_pass87 counts: it is lesson depth + missing banks + a Fill button that never reported success.
• Image-post crash is preflight CSS (see 2), not a missing chunk; do not re-add `require('tailwindcss')` unscoped.
• Own posts ARE savable (`showActions` defaults true); the real fault was hydrate clobbering — fixed by merge, not by forcing a hydrate per mount.
• `@react-navigation/native` is not a direct dep (TS2307) — use expo-router's `useFocusEffect`.
• The web/raw and gh-pages builds share ONE entry filename; tell the flavours apart by grepping for the string `deenapp` count.
...
## PASS 96 — workspace budget (173 MB → 87 MB) and the dumps that were being served

### Why the workspace was over budget

Nothing in the source trees had grown: the **deploy clones** had. `scripts/ship.sh` re-clones
`deenlink-api@main`, `deenapp@master` and `deenapp@gh-pages` into `../deploy` on every ship and
never removed them — 82 MB, of which gh-pages alone is ~18 MB because it keeps every old chunk by
design. Because `.git` is dropped between sessions they cannot even be fetched from — they were
pure ballast, and they came straight back after every second pass.

* `ship.sh` now removes the clones it created once the ship is done (skipped when `--eas` is
  running a build out of one of them) and prints the real snapshot size. The clones are re-created
  on demand, so nothing is lost.
* Removed by hand this pass: the three gitless clones (82 MB), a stale scratch copy of
  `profile.tsx` and `[username].tsx` at the workspace root (the real screens live in
  `deenapp/src/app/…` and `deenapp/src/app/profile/[username].tsx`), and the stray archives
  `api.zip`, `admin.zip`, `deenlink_db (9).sql`, `api/deenlink_db.sql`, `api/deenlink_db (10).sql`.

**Never delete to save space:** `deenapp/assets/content/**` (the hadith and Qur'an corpus the
bundle `require()`s), `deenlink-api/uploads/**` and `deenlink-api/storage/**` (user content and
server state, gitignored — not recoverable from origin), and the raw-export copy under
`deenlink-api/` (it is the cPanel docroot and is regenerated by `scripts/export-raw.sh`).
`node_modules`, `dist`, `.expo`, `.npm` do not count against the snapshot budget.

### The silent ship (worth remembering)

The first run of this pass printed `deenlink-api@main: no changes to commit` and finished with no
error — **rsync had disappeared**, because apt packages do not survive between sessions. The copy
step failed, `commit_push` found nothing staged, and the pass looked shipped while nothing had
moved. `ship.sh` now installs rsync if it is missing and fails loudly if it cannot. If a ship ever
reports "no changes to commit" for work you know you did, check rsync first.

### A database dump was downloadable from the live site

While trimming, five files in the web root turned out to answer HTTP 200 to anyone who guessed the
name — verified live, not inferred:

| URL | what it was |
| --- | --- |
| `https://app.deenlink.org/deenlink_db (9).sql` | 2.4 MB SQL dump of the database |
| `https://app.deenlink.org/api/deenlink_db.sql` | 60 KB dump |
| `https://app.deenlink.org/api/deenlink_db (10).sql` | 102 KB dump |
| `https://app.deenlink.org/api.zip` | 696 KB source archive |
| `https://app.deenlink.org/admin.zip` | 379 KB source archive |

Both repos: `deenlink-api` is **private** (so GitHub was never the exposure), `deenapp` is public
by design. The fix is a `FilesMatch` deny in the root `.htaccess` (same `Require all denied`
directive the host already honours — `api/config/` answers 403 live). **`.gz` is deliberately
excluded from that list**: `translations/*.json.gz` and the hadith corpus are fetched at runtime.

Verified by execution, not by reading the config: apache2 was installed locally with
`AllowOverride All` on a copy of the docroot, the four offender names were recreated, and the
before/after was measured — offenders `200 → 403`; `.gz`, `/index.html`, `/tools/scholars.html`,
`/islamqa.json`, `/adhan/*.mp3`, `/assets/**` and the fonts all stayed `200`; `api/config/private.pem`
(the private key in the repo) stayed `403`.

Shipped as `deenlink-api@main ba2f160` — it rides along with the pass-95 Deploy HEAD. The files
themselves are still on the server (a push cannot delete them: "merge, never prune"), so they are
merely unreachable now. Deleting them in cPanel → File Manager is still worth doing, and removing
them from the private repo needs a deliberate one-off (the ship script refuses deletions by
design).

## PASS 96 — the avatar library, date of birth, and the aqeedah rules

### 1. The avatar pictures were the WRONG FILES (root cause found)

The owner: "the ones youre using, they are not mine at all". He was right, and the reason is worse
...
Every layer "worked", which is why this survived two passes: same count, same names, valid JPEGs,
and the bundled/app-server copies matched *each other*, so any check that compared the app against
the server passed. Only the origin archive disagreed.
...
`api/users/list_profile_avatars.php` had a comment promising a session fallback and code saying
`$gender = 'male'` — so **any** caller that did not pass `?gender=` got the male library, and a
female account's list then had every row filtered out by the app's own gender guard (an empty
grid). Proven in the rig before/after: female account, no parameter → 38 male rows before, 24
female rows after. The payload also never carried `gender`, which the app's filter reads.

Also removed from the same endpoint: an invented price list ("the last N images are locked at 30
points") that ignored the admin's own `price_points` whenever the database was unavailable, and
rows whose file is missing on disk are now skipped instead of rendering a broken square.

### 3. Avatars wired in the admin — upload, price, and a delete that finally sticks

Deleting an avatar did nothing: `avatars_sync_files()` re-inserted any file it found on disk, so
the row came back on the very next request. There is now a `removed_from_library` tombstone
(guarded ALTER, DDL outside any transaction) that the sync respects. Verified end to end: admin
uploads a male avatar at 25 points → file lands in `img/profile/male/`, serves HTTP 200, appears in
the member's list locked at 25 → admin deletes it → still gone after two sync passes (1 row,
tombstoned). The section is labelled in Admin → Wallpapers Management ("Profile Avatars — app
pictures, upload & DeenPoints price; price 0 means free"), which is where the upload + price form
already lived.

The app's picker now spends the points properly: a locked tile shows a `60 pts` chip, tapping it
calls `unlock_profile_avatar.php` (idempotent; the server prices it), then selects — and if the
balance is short it says so instead of failing silently. Every tile loads through a spinner
("the images should have loaders case of network") and falls back to a placeholder icon on error,
and the sheet itself shows a loading state instead of an empty grid. Rig proof: 100 points → price
one avatar at 60 → select attempted → unlocked → `spent=60`, balance 40, `profile_image` updated.

### 4. Date of birth on every new registration

`users.date_of_birth DATE` (guarded ALTER), required and validated on both sides: real calendar
date, not in the future, 13–120 years. The app has the field on the normal sign-up form and on the
Google "complete your info" form, with an inline message as you type. Rig: no DOB → refused, future
date → refused, a 10-year-old → "You must be at least 13 years old", `1996-04-17` → stored.

### 5. Aqeedah — selected already, no description on Edit profile, weekly change, admin list

* **It was impossible to pre-select.** `/api/auth/me.php` never selected or returned `aqeedah` at
  all, so Edit profile always opened with nothing chosen. `/me` now returns `aqeedah` and
  `aqeedah_change_allowed_at` (absolute date), and the screen pre-selects the saved value — a
  custom one lands on **Other with the text already in the field**.
* **Descriptions removed on Edit profile** (`showDescriptions={false}`) plus the helper sentence I
  had added in pass 94. Registration keeps name + explanation, as designed then.
* **Once a week**: was 30 days with a message that never said when. Now 7 days, and the refusal
  carries the date; the screen greys the list out and states "You can change it again on 27 Sep
  2026" *before* he taps. Rig: change → allowed, window set +7d; immediate second change → refused
  with the date; stamp moved 8 days back → allowed.
* **The admin now sees them**: Admin → Aqeedah & Security has "Accounts with their own aqeedah"
  listing every account whose value is not one of your options (deleted/tombstoned accounts and
  junk like "Unknown"/"-" filtered out). Each row can be given one of your options or a typed one,
  or removed (empties the value so he chooses again). Wording shared by several accounts is
  grouped — "Sunnah × 5", "Sunna × 3" — with one click to fix the whole group: proven, 5 accounts
  rebadged and 3 cleared in the rig. An admin correction deliberately does **not** start the
  member's weekly cooldown.

### Verified by execution

`tsc --noEmit` 0 · `php -l` 476 files, 0 failures · inline admin JS parsed with `node --check` ·
the rebuilt rig (MariaDB 11.8 restored from `deenlink-api@main` + PHP 8.4) exercised every item
above with the app's exact request shapes. The rig itself had to be rebuilt this pass: MariaDB, the
