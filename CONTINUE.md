# PASS 22 SHIPPED (read first)
Pass 22 live (master af21bcd, gh-pages entry-e2e8e8): MUSHAF v3 — basmallah ONLY at true surah starts (numberInSurah===1, never 1/9, no سورة double-prefix; API page 2 starts Baqarah so 1 basmallah there is CORRECT), compact centered surah-name pill + bold basmallah, page FILLS screen (no prev/next row), browsing never hijacks audio, playing audio AUTO-TURNS pages (FOLLOWING RECITATION), continuous 13-26pt SizeBar + overflow scrolls INSIDE page, dark app theme defaults Night skin (dl.mushaf.prefs2), swiping across surahs router.replaces the reader (sync). AUDIO: double-buffered engines (next ayah preloaded = seamless) + progress polling 400ms + loading state + PAUSE on ayah buttons + domEnsurePlay() DOM-level fallback (expo-video web play() silently aborts after replace(); its `playing` flag lies true — see QuranAudioContext/useAudio). GLASSY GlassPlayerBar (blur/backdropFilter) shared by reader/dua/athkar(names; reader subtitle UNDER title, seek shortened via seekMargins). DUA v2: full dataset shape (LANGUAGE_ARABIC_TRANSLATED_TEXT=transliteration, TRANSLATED_TEXT=translation, REPEAT, per-part AUDIO https://hisnmuslim.com). 99-NAMES: content pack has per-name audio → https://islamicapi.com{path} (200 OK), plays via glass bar. POSTS VIDEO: draggable PanResponder seek + glass container + single VideoView attach (modal owns the player while expanded — fixed preview-disappear/crop). QURAN TAB: saved-ayahs row moved INTO Bookmarks filter; arabic surah names 23pt. SHARE: friends always visible (no toggle), shareCard auto-shrinks long text (92→56 / 44→28 steps, caps 9/8 lines + ellipsis), daily ayah/hadith pickers skip long entries. QUIZ: trophy always + Confetti component when 100%. INBOX v2 (components/CommunityInbox.tsx, chat NOW allowed per user): reels/posts/duas/ayahs shares + chat-back + quick in-app shares + emoji reactions, 'dl.inbox.v2' persistence; routes /tools/inbox (standalone) + community bell; quick-access keys 'inbox','ai'. DEENLINK AI /tools/ai: on-device intents over OUR quran/dua data (hyphen-tolerant surah match) with deep links + history chips. QIBLA: real kaabah icon (cube+gold band+door), rotation chevron arrows, aligned glow; RouteMap → GLOBE (equirect plot of user+Kaaba, gradient thread arc, graticule). COMMUNITY: SuggestStrip every 5th card while scrolling; /tools/suggestions (View more); /tools/connections Instagram-style Following/Followers/Suggested from profile stats; profile-preview @deenlink pill moved to TOP-right. HADITH: chapters synthesize from book data when meta missing (nawawi40 = 1 chapter, chapter_number null→1), hero exactly quran-style single green bar. SPLASH theme-aware bg (light theme no black). Verified: scripts/diag22.mjs 24/24 PASS (incl. audio actually playing + seek fill % + basmallah counts). NOTE: sandbox restart mid-pass rolled local git back — rebase conflicts resolved with --theirs (ours=remote); tsc clean after.

# PASS 21 SHIPPED (read first)
Pass 21 live (master 750ce77, gh-pages entry-b983b4): single-ayah tap now plays ONLY that ayah (was nested-pressable firing playSurah); announcement 3s→5s with live countdown chip, then router.replace to the NEXT SURAH reader (announcedNext ref); reader player compacted (title+reciter one line, seek 16px); MUSHAF ayahs justified + drag SizeBar (SIZES snap, live pt readout) replaces 7 buttons, page-number no longer collides; quran tab: ONE unified search (surah or ayah) via ContentSearchOverlay with ?ayah= deep-link flash+scroll, SAVED AYAHS row always visible when bookmarks exist; HADITH white screen FIXED (Bukhari english is {narrator,text} object → React #31; enOf() normalizer in both screens) + CONTINUE-READING hero (dl.hadith.last global pointer, deep-links ?chapter=); QUIZ no mid-question reveal (silent submit, 420ms auto-advance, results at end only); SEERAH v2 from seera_events_en.txt (142 events w/ location+source, search, bookmarks 'dl.seerah.marks', read-more modal); COMMUNITY INBOX via bell (components/CommunityInbox.tsx — friends→shared posts/reels/ayah-clips threads, double-tap/React emoji panel, NO CHAT); posts share = ContentShareSheet (kind 'post', link /post/{id}) + video seek fixed via 500ms currentTime polling; shareCard returns blob: URLs (mobile Safari can't render multi-MB data: URLs) + optional arabic; SPLASH now shows on EVERY cold page load (sessionStorage once-per-tab gate was the "not working" bug — iOS Safari keeps tabs alive for days; verified headless fresh+reload); LOGIN uses real logo (assets/img/logo-badge.png from deenlink.org) + slogan "All-in-one islamic app". Verified: scripts/diag21.mjs 11/11 PASS.
